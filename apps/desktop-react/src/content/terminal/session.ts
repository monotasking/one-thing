...
export class TerminalSession {
  ...
  /**
   * **程序化写入**(2026-09-14,代码块那颗「运行」)。
   *
   * 它直转 `send` —— 也就是说**与键盘按下去走的是同一条直达 PTY 的路**,同样
   * 不过资源管线。那段判词已经写在上面 `screen.onData` 的注释里(一次按键要在
   * 几毫秒内到达 PTY,而管线那条路上有授权、审计、投影三站);这里再开一条
   * 「稳妥些的慢路」只会让同一格终端有两种到达顺序。
   *
   * 所以 `exited` / `dead` 之后同样一个字节都不发(判据在 `send` 里,只有一处)。
   * 模型操作终端仍然是 T3 那条另外的路,两条在**后端**汇合,不在这里。
   */
