 ...
 *
 * ── 六条自述,每一条各替掉一处本来要写死的判据(W5-c-3 之后是**五条** ——
 *    `fullable` 那一条撤了,见下)────────────────────────────────────────────
 ...
 *  · ~~`fullable: false`~~ —— **W5-c-3 撤掉了**,与会话那一种同一条理由、同一次
 *    裁定:它从前的理由是「路线 B 下输入框仍在树外,而两格里随时可能有一格是
 *    会话」,而路线 A 之后输入框是会话叶自己的器官 —— 一格 `pair` 就是两片会话
 *    叶,两块输入框跟着各自那半格一起进全屏。缺席 = 进得了。
