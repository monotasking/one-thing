 ...
 * | 行内「打开」 | **不存在**(P5 量出来的:Obsidian CLI 没有「只把库调到前台」的动词);「在 app 里打开」的落点是检索结果那一行 |
  ...
  return (
    <div className={off ? `${s.vaultRow} ${s.vaultRowOff}` : s.vaultRow} data-testid={`notes-vault-${vault.id}`}>
      ...
      <div className={s.vaultActions}>
        ...
        {/*
          **这一行上没有「打开」钮,而那是量出来的**(P5,2026-09-18):Obsidian CLI
          整张动词表里没有「只把某个库调到前台」这件事(`open` 硬性要 `file` 或
          `path`),所以那颗钮按下去只会失败。P4 留账 2 由此结清 —— 「在 app 里打开」
          今天唯一真做得到的落点是**检索结果那一行**(它手上有一份笔记的路径)。
          真要一颗「把这个库调出来」,得走 `obsidian://open?vault=` 那条 URI,而 React
          壳今天的 `shell` 端口是 `null`。判词写在 `@shared/ipc/notes.ts` 上。
        */}
      </div>
    </div>
  )
