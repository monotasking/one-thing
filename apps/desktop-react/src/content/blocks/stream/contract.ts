...
export type BlockGeometryPolicy =
  ...
  | 'flow'
  /**
   * 先占位再填内容(表的列宽锁定、图的内在比例)。
   *
   * R4a **不实现任何 reserve 行为**(本期保行为等价);这一格是给 R4b 的骨架政策
   * 与将来的媒体型留的答案位,今天所有型都答 `flow`。
   */
  | 'reserve'

/**
 * 「早成形」政策 —— **正在出生的那一型自己认领活尾巴**。
 *
 * 从前这是增量解析器里一段写死的 `upgradeTableTail`:它认识表、认识分隔行、认识
 * GFM 的列数规矩。R4a 把它翻成一问 —— 由**表自己**声明「这段活尾巴补成什么样能
 * 让解析器提前一步认出我」,机制只负责:补一刀 → 重解析一次 → **末块真的是声明者
 * 那一型才认**,否则整段作废、一个字不改。
 *
 * 于是机制层对「表」二字一无所知,而将来任何一型(嵌套 JSON、公式、自定义围栏)
 * 想提前成形,代价是它自己的注册契约里多一行。
 *
 * @param source 活尾巴那一块的源文本(贴着流末尾的那一段)
 * @returns 补齐后的源文本;认不出就 `undefined`(最坏情况白算一次)
 */
export type BlockEarlyForm = (source: string) => string | undefined
...
export interface BlockStreamContract {
  ...
  /** 可选的第六格:早成形。没有就是「等解析器自己认出来」。 */
  earlyForm?: BlockEarlyForm
}
