...
import { SEAT_MIN_LINES, seatHeight } from '../seat'

/**
 * 座位几何的四支(正本 `docs/send-flow-2026-09.md` §3)。
 *
...
const MIN = LINE * SEAT_MIN_LINES // 67.2
...
describe('座位几何', () => {
  it('① 常态:能用的地减掉这一轮已经长了多高', () => {
    // room = 760 − 24 − 100 = 636;气泡 40 高,回复还没开口(tail = 气泡本身)。
    expect(seatHeight({ ...BASE, userHeight: 40, tailHeight: 40 })).toBeCloseTo(596, 5)
    // 回复长了 200 —— 座位缩掉同样多,`scrollHeight` 一格不变。
    expect(seatHeight({ ...BASE, userHeight: 40, tailHeight: 240 })).toBeCloseTo(396, 5)
  })

  it('① 长满就归零,不会变成负数(之后退役为普通跟底)', () => {
    expect(seatHeight({ ...BASE, userHeight: 40, tailHeight: 636 })).toBe(0)
    expect(seatHeight({ ...BASE, userHeight: 40, tailHeight: 5000 })).toBe(0)
  })

  it('② 三行下限:气泡快把地占满时,起手仍留三行', () => {
    // room − userHeight = 636 − 600 = 36 < 三行(67.2)→ 兜到下限。
  ...
  it('④ 自己那条比视口还高:上缘落置顶线,座位取下限', () => {
    expect(seatHeight({ ...BASE, userHeight: 2000, tailHeight: 2000 })).toBeCloseTo(MIN, 5)
  })

  it('读数里混进 NaN 时当 0 算,不把 NaN 写到样式上', () => {
    // 气泡量不到 → 当 0 高:起手 636,气泡之后那 40 照旧算长出来的 → 596。
    expect(seatHeight({ ...BASE, userHeight: Number.NaN, tailHeight: 40 })).toBeCloseTo(596, 5)
    // 一行有多高量不到 → 下限算不出来 → 老实答 0(退化,不是猜一个数)。
