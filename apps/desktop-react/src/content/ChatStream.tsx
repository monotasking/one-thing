...
function useFollowBottom(
   ...
   * **量与写的唯一那个口**(G 线 P2-a,`content/viewport/scroll-port.ts`)。
   *
   * 从前这只 hook 自己拿着 `scrollRef` / `seatRef` 两格 ref 读写 DOM;收口之后
   * 「写 `scrollTop`」只剩 `port.setTop` 一处,停靠守卫与 `lastTop` 同步都在那口
   * 里做一次 —— 判词整段在那只文件的头上。
   */
  port: ScrollPort,
  sessionId: string,
  ...
  /*
   * ── 「人此刻停在第几像素」搬进 `ScrollPort` 了(G 线 P2-a)──────────────────
   * 从前是这只 hook 里的 `lastTopRef`,写点五处(滚动回调、`stick`、折叠补偿、
   * 插值的每一帧),每一处都要记得跟着写一句。收口之后**它跟着那个写口走**:
   * `setTop` 写完当场记,`noteScrolled` 在滚动那一头记 —— 判词整段在
   * `content/viewport/scroll-port.ts` 的文件头(纪律 ②)。
   * 读点仍是两处:取回那一拍(`useParkedScroll`)与「是谁离的底」那一句。
   */
   ...
   *
   * **停靠中不写**那一句现在由 `setTop` 统一做(`measure()` 在停靠中答
   * `undefined`,于是这里连 `scrollHeight` 都读不出来)—— 判词照搬:
   * `content-visibility: hidden` 的子树里没有盒子,`scrollHeight` 与 `clientHeight`
   * 一律报 0,赋一次 `scrollTop = 0` 就是把这条会话的位置抹掉,而人只是切去看了
   * 别的会话。判据用几何(`clientHeight === 0`)而不是「宿主说我看不见」:它对
   * 任何一种藏法都成立,与 `tools/card-heights` 那句 `height <= 0` / clamp-measurer
   * 同一把尺子。贴完当场记 `lastTop` 那一句同理,搬进了那个写口。
   */
  const stick = useCallback(() => {
    const m = port.measure()
    if (!m) return
    port.setTop(m.scrollHeight, 'tail-growth')
  }, [port])

  /** 「下面还有多少没露脸」。判据与 `follow.ts` 的 `scrolled` 用的是同一个式子。 */
  const readGap = useCallback(() => port.gapNow(), [port])
  ...
  useLayoutEffect(() => {
    ...
    const observer = new ResizeObserver((entries) => {
       ...
       *
       * G 线 P2-a:这一句连同「把这一批归一成纯数字」一起搬进了
       * `ScrollPort.summarizeResize` —— 它**先答这一句再读任何矩形**,次序与
       * 今天逐字相同;答 `undefined` 就是「停靠中」。
       */
  ...
  const onScrollWithFollow = useCallback(() => {
    ...
    if (el) {
       ...
       */
      const { previousTop, top } = port.noteScrolled()
      /*
    ...
    scheduleAnchorSave()
    onScroll?.()
  }, [scrollRef, port, onScroll, dispatch, scheduleAnchorSave])

  /*
   * **拿回来那一拍**(2026-09-10 组件级停靠)。判词整段在 `useParkedScroll` 上;
   * 它要的那两格镜像是这只 hook 的内脏,所以调用点在这里,不在组件层。
   */
  useParkedScroll(visible, port, unparkTopRef)
...
function useParkedScroll(
  ...
  useLayoutEffect(() => {
    ...
    if (came) unparkTopRef.current = port.lastTop
  }, [visible, port, unparkTopRef])
}
