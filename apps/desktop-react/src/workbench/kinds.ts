...
export interface ContentKind {
   ...
   *
   * ── 今天**没有一种说 false**(W5-c-3 起)────────────────────────────────
   * 从前 `session` 与 `pair` 两格说 false,理由是同一条:「输入框挂在外壳的
   * `.center` 上、不在树里,会话叶进全屏会把它盖掉」。路线 A 之后输入框就是会话
   * 叶自己的器官,跟着叶一起进全屏 —— 两格于是一起撤。
   *
   * **这一格因此仍旧有意义,而且不是死码**:`canGoFull` 对**认不得的种类**也答
   * false(说不出自己是谁的东西不该铺满整扇窗),所以「结构化拒绝」那条路照旧
   * 走得到;将来第二种真要拒绝时,写的仍然是这一行,不是核心层里的一条 if。
   */
  fullable?: boolean
  /**
