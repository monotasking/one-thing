 ...
 *
 * 侧栏那道阈值在仓里有四份:tokens.css 的 `--expose-rail-bp`、`Rail.module.css` 与
 * `Toolbar.module.css` 的两条 `@container` 字面量(`@container` 的条件里不能写
 * `var()`,那条判例写在 tokens 里),以及 W7-d 加的这一份 JS 常量(纯函数读不了 CSS)。
 * 四份**必须同源**,而「哪天忘了同步」不该由人记 —— 这只用例把真的 CSS 文本读进来比。
 *
...
describe('会话总览自述的浮窗下限(W7-d 裁定 1)', () => {
  it('阈值四处同源:tokens、Rail、Toolbar、这格 JS 常量', () => {
    ...
    // 顶栏那格选择器说的是「> 阈值就不必画」—— 所以字面量是阈值 +1。
    expect(css('src/expose/components/Toolbar.module.css')).toMatch(
