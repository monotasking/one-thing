...
export interface ProjectSummary {
  ...
  path: string
  /**
   * 这个项目下**最近一条会话的活动时刻**(A6 补的一格)。
   *
   * `buildProjects` 一直在算它(名册的次序就是按它倒序排出来的),从前只是没交
   * 出来 —— 排序把它用掉就扔了。范围菜单要按「近 7 天动过没有」折叠,判据只能
   * 是这一格:在屏幕上现猜(比如「排在前五的算最近」)是把次序当成时间,而
   * 一个项目排第三与它上周动过没有是两件事。
   */
  updatedAt: number
}
...
export interface ExposeState {
  ...
  query: string
  /**
   * **搜索行此刻是不是一只输入框**(09-12 方向 A:顶上没有常驻输入框,
   * 搜索是三行导航里的一行,点它才原地换形)。
   *
   * 它与 `query` 是两格而不是一格:开着而空着是一个真状态(刚点开、还没打字),
   * 而「有词」蕴含「开着」—— 收回去那一下连词一起清(见 `closeSearch`)。
   * 两格都**不落盘**(`EXPOSE_PER_SPACE.pick` 只拿三格家具):下次打开这块面
   * 该是一行字,不是上次留下的一只空输入框。
   */
  searching: boolean
  /**
   * **哪一行此刻正在原地改名**(A2;null = 没有)。
   *
   * 它与 `searching` 是同一族的一格:一件**只活在这块面上**的临时形态,
   * 所以同样**不落盘**(`EXPOSE_PER_SPACE.pick` 只拿三格家具)。一屏至多一行
   * 在改名 —— 所以它是一个 id 而不是一张表:两行同时改名没有语义(键盘只有
   * 一个焦点,而「改到一半点开第二行」的正确行为是前一行收回)。
   *
   * 草稿**不在这里**:那一格由长在那一行上的输入框自己持有(它挂载时从标题
   * 起步、卸载时随它一起没)。落进 store 的话会多出一格「改名已经关了、草稿还
   * 留着」的状态,而它对屏幕没有任何意义。
   *
   * 归位时清掉(`open` / `enterSession`):点进一条会话或这块面重新摆出来之后,
   * 屏幕上不该还留着一只开着的输入框(与 `searching` 逐字同一条理由)。
   */
