 ...
 *
 * ── 为什么 settings 也在这条端口上 ───────────────────────────────────────
 * 「哪一家可见」的两道闸(开关、选过哪些模型)读的是 `AppSettings.ai`:
 * `providers[pid].enabled`(经 `isProviderEnabledIn` 的家族派生)与
 * `providers[pid].selectedModels`。它不是模型域的口,但它是 models-source
 * 唯一多出来的取数面 —— 为它单开一个 settings-port 只会多一份要在
 * `src/test/setup.ts` 里维护的假端口,而这一层端口的存在理由本来就是
 * 「这个数据源要摸的那几口」。真长出第二个 settings 消费者时再拆。
 *
 * ── 留账:两道闸本批不做 ─────────────────────────────────────────────────
 * Vue 壳的模型选择器筛三道(`ModelSelector.vue:293-315`):
 *  1. `isProviderEnabledIn`(开关,含家族派生)—— 本批做;
 *  2. 有模型可列(`selectedModels` 非空)—— 本批做;
 *  3. `isConfigured`(这个空间配了凭证没有)—— **本批不做**。它的判据在
 *     `useSpaceProviderView` 里,吃的是 space 的 `providers.json`,而新壳今天
 *     没有 space 视图,凭证探测口也没有勘察过。诚实的降级是:配错了家的模型
 *     选出来能选中,发消息时才失败 —— 屏幕不假装它知道凭证在不在。
 * 同理 **per-space 覆盖**(`ProviderEnabledOverride`)也不传:新壳没有当前空间
 * 这个事实,传 `undefined` = 回落全局,与 Vue 壳的默认空间逐字同义。
 */
export interface ModelsPort {
  ...
  /** 整份应用设置。只读两格:`ai.providers[*].enabled` 与 `.selectedModels`。 */
...
async function realPort(): Promise<ModelsPort> {
  const [{ providersApi }, { modelsApi }, { settingsApi }, { sessionsApi }, { whenConnected }] =
    await Promise.all([
