import type { GetSessionTokenUsageResponse } from '@shared/ipc/sessions'
import type { GetSessionUsageResponse } from '@shared/ipc/usage'

/**
 * 读数(context / tokens / 费用 / 缓存)与 `@renderer/platform` 之间的那一层
 * **端口** —— 与 `data/models-port.ts` 同一形状、同一理由。
 *
 * 两口,恰好是这四行读数的**两个真产地**:
 *  - `platformApi.getSessionUsage` —— 计费账本(tokens 六格 + 三种成本);
 *  - `sessionsApi.getTokenUsage` —— 会话上的 token 读数(contextSize /
...
async function realPort(): Promise<MeterPort> {
  const [{ platformApi }, { sessionsApi }, { whenConnected }] = await Promise.all([
  ...
  return {
    ready: () => whenConnected(),
    getSessionUsage: (sessionId) => platformApi.getSessionUsage({ sessionId }),
