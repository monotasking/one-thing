...
export const PERF_BUDGET = {
  ...
  /**
   * **切焦点叶**「按下 → 稳定」的 p95 上限,毫秒(W5-b 裁定 9:场景⑤分裂成两半)。
   *
   * 与 `sessionSwitchMs` 是两件事:那一格是「同叶换 ref」——那片叶的内容整份换人,
   * 折叠、装配、排版全要重来;这一格是「两片叶都已经在屏上,只把焦点(与『当前会话』
   * 那条投影)指过去」,理应几乎不花钱。所以它**不是** 393 的复制,而是自己的一格。
   *
   * **首次读数**(2026-09-05,本机 `gate:perf` 场景⑤b,两趟各 8 次切换):
   * p95 **14ms** / **16ms**(逐次 `12 13 14 12 12 12 13 12` 与 `10 16 15 14 16 14 14 14`)。
   * 预算取 60ms —— 与 `sessionSwitchMs` 同一条立法理由:时间会随机器抖,所以它一起断言
   * 但不是唯一红绿线(真判据是下面那格强制排版次数),留的余量只要能接住机器噪声、
   * 又**接不住真回归**即可:投影那条订阅一旦把整棵中央树重渲一遍,这一格会跳进 ⑤a 那个
   * 数量级(百毫秒),60ms 当场拦下。
   */
  focusLeafSwitchMs: 60,
  /**
   * **切焦点叶**单次允许出现的强制排版次数上限(同上那条裁定)。
   *
   * 这一格才是 ⑤b 真正的红绿线:整数计数,不随机器噪声抖。数字是门在这台机器上量出来的
   * **确定性读数**(8 次切换里的最大值),不留余量 —— 与 `sessionSwitchForcedLayouts`
   * 同一条立法理由。首次读数两趟**逐字相同**:`0 1 1 1 1 1 1 1`(第一次那下是 0,因为
   * 那片叶刚被点亮、还没有可被问询的旧布局),最大 1。投影那条订阅要是把整棵中央树重渲
   * 一遍,这一格当场变红。
   */
  focusLeafSwitchForcedLayouts: 1,
