 ...
 *
 * ── S4b:范围从「骨架」收紧到**整个 `src/search`** ────────────────────────
 * S4a 那时豁免三处,其中一处是 `sources.ts`(「壳对 chats / messages / files 另有
 * 自己的产地」这个事实本身)。**那个文件本批删了** —— 壳这一侧只剩一条
 * `search.query`,三类的产地全在后端。于是豁免只剩两处:
 *
 *  1. `targets/<kind>.tsx` —— 一种目标形一个渲染模块,§4.0 允许动的那两处之一。
 *     它们主要写的是 `target.kind` 不是能力 id;**S4b 起有两个文件里真的出现了
 *     能力 id**(`chat.tsx` / `file.tsx` 的枢轴要说「跳到消息那一档」)。那是
 *     刻意的,理由写在那两个文件与 `../continuations.ts` 头上:「以谁为词、跳到
 *     哪一类」只有产这条结果的那一类知道,而契约今天装不下「后端自报一条
 *     `kind:'continue'` 的动作」(`SearchActionDescriptor` 缺 `kind` / `payload`
 *     两格)。补上那两格之后连渲染器都不用认识它 —— 那是另一批。
 *  2. `__tests__/**` —— 用例当然要点名具体的能力,那正是它们在验的东西。
 *
 * **面板 / tab 条 / 分组 / 分页 / 过滤片 / 预览窗 / 状态行一个 id 都没有**,
 * 那正是这道闸此刻在守的东西。
 *
