...
export const zh = {
  ...
  'memory.released': '刚才释放了 {count} 项,约 {bytes}',
  'memory.releaseFailed': '释放失败:{reason}',
  'memory.readFailed': '读取失败:{reason}',
  'memory.loading': '正在读取…',
  'memory.totalUnknown': '总量未知',
  'memory.pressureOk': '预算内',
  'memory.pressureSoft': '超过 soft 线',
  'memory.pressureHard': '超过 hard 线',
  'memory.pressureUnknown': '未知',
  'memory.partial': '有进程量不到,总量偏小',
  'memory.processes': '进程',
  'memory.noProcesses': '量不到任何进程',
  'memory.unmeasured': '量不到',
  'memory.holders': '缓存',
  'memory.unitSessions': '{count} 个会话',
  'memory.unitEvents': '{count} 条事件',
  'memory.unitViews': '{count} 个网页',
  'memory.releaseHint': '让各个缓存把能重建的都放掉:后台放久了的网页、空闲的会话缓存。之后第一次打开会慢一点。',
  'memory.budgetLabel': '内存占用与预算',
  'memory.heapLine': 'core 的 JS 堆 {heap} / {total}',
  'memory.overview': '内存总览',
  'memory.totalLabel': '总占用',
  'memory.trendWindow': '最近 5 分钟',
  'memory.processCount': '{count} 个',
  'memory.trend': '最近的走势',
  'memory.trendWarming': '再等几秒就有曲线了',
  'memory.trendSummary': '最近 {minutes} 分钟 · 最低 {low} · 最高 {high}',
  'memory.agoNow': '刚刚',
  'memory.agoSeconds': '{count} 秒前',
  'memory.agoMinutes': '{count} 分钟前',
  'memory.composition': '花在哪儿',
  'memory.catCore': '核心',
  'memory.catUi': '界面',
  'memory.catWeb': '网页',
  'memory.catSystem': '系统服务',
  'memory.catCoreHint': '主进程:会话、工具、检索索引与后台任务都在这里',
  'memory.catUiHint': '这扇窗口本身的渲染进程',
  'memory.catWebHint': '内置浏览器打开的网页,每一格一个进程(页里的跨站框架各算一个)',
  'memory.catSystemHint': 'GPU、网络、音频这类 Chromium 服务进程',
  'memory.holdersHint': '可以重建的缓存。超出预算时会自动释放,也可以点右上角手动释放。',
  'memory.holderFill': '{name}:已用 / 上限',
  'memory.nameCore': '主进程',
  'memory.nameNetwork': '网络服务',
  'memory.nameAudio': '音频服务',
  'memory.nameStorage': '存储服务',
  'memory.detailSessions': '{count} 个会话',
  'memory.detailSubscribed': '{count} 个在看',
  'memory.detailCapacityPerSession': '每个会话最多 {count} 条',
  'memory.detailIdle': '空闲 {count}',
  'memory.detailProtected': '在用 {count}',
  'memory.detailTabs': '共 {count} 个标签页',
  'memory.detailHidden': '在后台 {count}',
  'memory.detailAudible': '在出声 {count}',
  'memory.detailHibernated': '已释放过 {count} 次',
