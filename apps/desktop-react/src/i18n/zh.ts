...
export const zh = {
  ...
  'memory.releaseHint': '释放可重建的缓存,包括后台网页与空闲会话。释放后首次访问需要重新加载。',
  'memory.budgetLabel': '内存占用',
  'memory.heapLine': '主进程 JS 堆 {heap} / {total}',
  ...
  'memory.catWebHint': '内置浏览器中的网页,每个标签页及其跨站子框架各占一个进程',
  'memory.catSystemHint': 'Chromium 的 GPU、网络、音频等服务进程',
  'memory.holdersHint': '可重建的缓存。内存超出预算时自动释放,也可手动释放。',
  ...
  'memory.detailSubscribed': '已订阅 {count}',
  'memory.detailCapacityPerSession': '每个会话上限 {count} 条',
  'memory.detailIdle': '空闲 {count}',
  'memory.detailProtected': '使用中 {count}',
  'memory.detailTabs': '标签页 {count}',
  'memory.detailHidden': '后台 {count}',
  'memory.detailAudible': '播放音频 {count}',
  'memory.detailHibernated': '已释放 {count} 次',
  'memory.hardLine': '硬上限',
  'memory.softLine': '软上限',
