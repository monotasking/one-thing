...
/**
 * --dur-preview-switch:预览泡**已经开着时**横向换到旁人身上的接手延迟
 * (09-01 用户体感裁定:「切到另一块瓦,原来的 preview 还在」)。
 *
 * 与 PREVIEW_DELAY_MS(首开 600)是两件事,菜单 / 预览的通行语义:第一次打开要
 * 沉住气(擦过去的不算),**开着之后切换要跟手** —— 屏幕上已经有一块在讲话了,
 * 再让人等 600ms 只会看见「旧的还赖着」。100 只挡真正的一擦而过。
 * 瞄准三角区武装时(朝泡走)照旧不换人,与这一档不冲突。
 */
export const PREVIEW_SWITCH_MS = 100
/**
...
/**
 * --dur-dock-reentry:自动隐藏的**回身窗口**(09-01 修「缝里来回闪」)。
 *
 * 出来的边界(8px 窄带)与收起的边界(整块留驻区)差了 200 多像素,中间那片
 * 是**死区**:真机逐像素量到,掉出去之后站在条身正中都叫不回来,得一路怼到
 * 离屏底 8px 才行(死区高 211px)。迟滞因此要有时间这一维:刚被自动收起的
 * 这一小会儿,「回身」认留驻区;过了就退回窄带,于是隔了半天才去打字的手
 * 照样叫不醒它(08-31「自动出现范围太大」那条不回退)。
 *
 * 1200 覆盖一次「咦?没了 —— 手回去」的修正动作(看见 + 反应 + 移动,~250…600ms)
 * 还留着一倍余量,同时远短于「走开一趟再回来」。
