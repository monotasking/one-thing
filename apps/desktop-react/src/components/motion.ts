...
/* ── 唱片的惯性(宠物 P1 起;音乐面 v8 起只剩这一件)─────────────────────────────
 *
 * 角速度只在 rAF 里改(不进 React 状态),起转 / 停转要多久是 JS 算的,所以要一份数;
 * 产地是 tokens.css,`__tests__/motion-tokens.test.ts` 逐条比对。v8 之前这里还有六个数,
 * 给「唱片收进封套 → 封套换歌 → 放片」那一串计时器用 —— 那一串随旧唱机场景一起删了。
 */
