...
describe('收起档的判据与几何(样式表源文本 + token 算术)', () => {
  ...
  /*
   * 左栏**只在「让开它之后目录还保得住五列」的宽度之上留着**。
   * 这条不等式是收起阈值唯一的理由 —— 定高了会在还排得下的时候把名册收掉,
   * 定低了就还原成报障那一形(268 恒在,面板 894 以下目录一路切到底)。
   */
  it('收起阈值不低于「左栏 + 详情内衬 + 五列入场」', () => {
    const needed = px('--pv-rail-w') + px('--pv-detail-inset') + px('--pv-catalog-bare')
