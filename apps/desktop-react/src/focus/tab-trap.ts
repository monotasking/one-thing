 ...
 *
 * 这两段是从 `ui/a11y/focus-trap.ts` **原样搬过来**的(R0 搬,R1 那只文件整个
 * 退役 —— Dialog / Menu / Popover / Palette 都改成了 `modal` 作用域)。
 * 搬而不引的理由:`focus-trap` 那只 hook 的形状是「装一个 document 监听 + 记一个
 * 锚点」,而树里需要的只是**判据**——「按 Tab 该停在哪」——由那一个派发器现问现答,
 * 不再多一个监听器;锚点那一半更是整个不需要了(归还是结构性的,§4.5)。
 * 所以这里就是那只文件唯一的遗骨。
 *
