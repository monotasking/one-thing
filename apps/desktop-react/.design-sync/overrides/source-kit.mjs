// forked from design-sync lib/source-kit.mjs - synth-entry must honor componentSrcMap:null
// (2026-09-16):这仓无 dist,入口由 srcDir 下每个 .tsx `export *` 合成;上游只把 null 掉的组件从
// **卡片列表**里摘掉,文件照样进入口。于是 NativeMenu.tsx(→ data/browser-port → platform/connection)
// 与 drag/*(→ workbench/store …)把整片壳的 zustand 仓与 keymap 拖进 bundle(215K → 769K),
// claude.ai/design 的自检在沙箱里加载它就死,组件面板为空。这份 fork 只多一条规则:
// 文件名(去后缀)等于某个 `componentSrcMap: null` 键的实现文件,不进合成入口。
// 相对 import 指回 .ds-sync/lib(fork 所在目录没有兄弟模块)。
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    // FORK: a file named after a `componentSrcMap: null` component stays out of the entry
    // (otherwise its import closure - host ports, stores - lands in the bundle).
    ...
    const comps = srcFiles.filter((p) => SRC_IMPL_RX.test(p) && !NON_IMPL_RX.test(p)
      && !excluded.has(p.replace(/\\/g, '/').split('/').pop().replace(/\.(tsx|jsx)$/, '')));
