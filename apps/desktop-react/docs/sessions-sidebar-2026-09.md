...
## 9. 范围菜单封顶 + 筛选行失焦收回(09-13 用户两条报障,A6)

**报障一:范围菜单无限长。** 范围行弹的 `ui/Menu` 列全部项目(真店 24 个),库件没有最大高度,违反壳规「浮层列表必有最大高度约束 + 选中项 scrollIntoView」。
拍板:
1. **封顶是库件的事**:`ui/Menu` 加最大高度(`--menu-max-h`,取视口高的一档,token 说明),超出在菜单内滚动,打开时选中项 `scrollIntoView({block:'nearest'})`。所有菜单一起受益。
2. **项目多时菜单顶上出筛选框**:项目数 > `EXPOSE_SCOPE_FILTER_MIN`(8)才画;打字即过滤,↓↑ 走、↵ 选、Esc 先清词再关;固定三档(全部 / 协作 / 无项目)钉在筛选框下,不参与过滤;项目按最近活动排(已有);近 7 天没动过的折成一行「更早 · N 个」,点开才展(有词时全部展开参与匹配)。

**报障二:筛选行变成输入框后,焦点走了不恢复。** 今天只在 Esc / 进会话时收回。
拍板:
3. 那一行改叫**「筛选」**(i18n `expose.searchRow` 文案改;它过滤的是这张列表,与 Dock 上的「搜索」瓦分清)。
4. **这块面离开活动路径**(焦点去了别的面 / 别的输入框)那一拍收回成一行:没词 → 与没点过一样;有词 → 行上显示「筛选 · <词> ×」(词 = 当前 `query`,× = 清词还原列表),点行重新展开输入框接着改。
5. 焦点回到这块面不自动展开输入框,落点是那一行;↵ / 点击才展开。
6. 判据走响应链:`ExposeView` 的作用域实例上订「活动路径是否还含 expose」的那一格(`focus/registry` 现有的 `useFocusScope` / `focusTree` 订阅),不加 keydown / focusout 监听,不读 `activeElement`。

门 `gate:sessions` 补:⑨ 范围菜单高度 ≤ `--menu-max-h` 且可滚、种 12 个项目时筛选框在场、打「lenovo」只剩一项、Esc 两档;⑩ 打开筛选行 → 点聊天输入框 → 行收回;带词再走 → 行显示词与 ×,点 × 列表复原。
