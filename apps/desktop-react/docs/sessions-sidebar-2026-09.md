...
2. **左缘 = 左侧红灯的中心**:`trafficLightPosition.x`(16)+ 半径 6 = **22px**。所有盒子(导航行、节头、行的悬停底色、顶部控件)的左缘从这条线起,**没有任何元素越过它**;文字在盒内再进 8(x=30)。用户原话:「所有元素从这个位置开始,不要超过它」。
...
[红灯中心 x=22 ────────────────────────────────]
   ...
   置顶                                节头 26px,文字 x=30,11px/600/.04em
   标题标题标题标题标题……        ●     行 32px,文字 x=30,单行省略,不画时间、不画项目 chip
   ⚡ 标题……                          形态图标**只在有图标的行**上画(14px + 6 间距),普通聊天不预留
   标题标题标题标题标   [⋯]           悬停:⋯ 从右盖上来,渐变 26px 收掉标题尾巴
```

- 页边距:左 `--content-lead-left`(22),右 8;`scrollbar-gutter: stable`(只右侧)。
- 行:高 32、内边距 0 8、间距 6、圆角 `--r-1`;当前会话 `--st-sel`,悬停 `--st-hover`,叠加 `--st-sel-hover`;键盘活动行照旧 outline 环。
- 打开状态点 `ui/OpenDot` 照旧,在标题右、动作左。
- 节头:高 26、上边距 10(第一节 4)、文字 x=30;折叠箭头在**行尾**,悬停或已折叠才显。
...
① 会话瓦钉到左架子、厚度拖到 240:`[data-shelf="left"]` 内所有可见盒子的 `min(left)` = 架子 x + 22,且红灯中心 = 同一个数(读 `--titlebar-traffic-w` 那一族与 main.ts 的 16 + 6 同源)。
