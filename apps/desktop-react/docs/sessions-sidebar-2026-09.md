...
2. **侧栏对齐律**(§8;09-13 用户三次画线裁定,第三次定稿):红灯中心 22 那条线穿过每一行 16px 图标列的中心;**每一行都有图标列**(普通聊天画淡灰对话气泡),整块面只有一条文字线 38;节名 14;盒 6。用户原话:「所有元素从这个位置开始,不要超过它」—— 「元素」指的是看得见的墨。
...
[红灯中心 x=22:图标列 14–30 的中心 ─────────────]
   ...
   💬 标题标题标题标题标题……     ●     行 32px,文字 x=38(盒 x=6),单行省略,不画时间、不画项目 chip
   ⚡ 标题……                          形态图标**每一行都有**(列 16 居中在线上,图标 14 + 一格 8);
                                      普通聊天画淡灰对话气泡 —— 09-13 §8 第 2 条把 09-12
                                      那条「只在有图标的行上画」翻了回来(A5)
...
│     └─ SessionTree
│        ├─ SectionHead × n(箭头在行尾)
│        └─ SessionRow × n(⋯ IconButton 一颗;glyph 每行都画(§8 ②);time / project 仅 ≥760)
...
## 8. 侧栏对齐律(09-13 定稿;比稿页 `docs/sessions-sidebar-align-proposal-2026-09-13.html`)

三次错法都被用户拿真机截图画线打回:A1 把悬停盒的左缘对上线(字再进 8);7174fefd 把墨的左缘对上线;472593c2 把图标左缘对上线而普通聊天没有图标列,于是导航文字在 38、标题在 14,两条文字线。定稿八条:

1. **一条竖线,由红灯定。** `trafficLightPosition.x` + 灯半径 = 22,token `--content-lead-left`。它说的是"线在哪",不是盒边或墨的起点。无红灯宿主(`[data-host-traffic='none']`)取 24。
2. **图标列居中在线上。** 每一行 16px 图标列(`--expose-glyph-w`),中心落在线上(14–30)。普通聊天画淡灰对话气泡(`--text-4`),房间 / 派工 / 代理照旧各自的形;不预留空列,也不省列。判据是中心对中心。
3. **一条文字线。** 导航行、会话标题、有图标的行,文字一律从图标列右边一格起(38)。
4. **节名退半格,条目往里进。** 节名从图标列左缘 14 起;层级靠缩进。
5. **盒子对称探出。** 悬停 / 选中底色左右各探出同样距离:容器左右内边距同数(线左 16 = 6),行内边距 8。
6. **檐上标签也进格子。** 「Sessions」标签的图标以线为中心。
7. **数从线推。** 容器内边距只许 `calc(var(--content-lead-left) - var(--expose-glyph-w) / 2 - var(--sp-2))`;6 / 14 / 38 不许出现为字面量。
8. **门量三件。** `gate:sessions` ①:导航图标与会话行图标中心 = 线;最左一笔墨 = 线 − 8;盒最左 = 线 − 16;没有墨越过图标列左缘。

代价:240 宽标题净宽约 166(报障时 83);400 行冷开多 400 个 svg,`gate:perf` 场景① 重量。

### A5 落地(2026-09-13)

八条逐条落地。**实施时挖出两处与本节字面不符的地方,按「线」这个判据判的**:

1. **第 3 条那条文字线要 38,而会话行的 gap 是 6**(导航行一直是 `--sp-2`)——于是屏幕上是
   36 与 38 两条线。治法是把 `--expose-row-gap-inline` 从梯外的 `6px` 改成 `var(--sp-2)`
   (38 = 14 + 16 + **一格**),顺带把它挪回间距梯上;总览形本来就是 `--sp-2`,那一档一个数没动。
2. **第 6 条的让位落在 tab 自己的内衬上,不是条的左内边距上。** `joined` 档的条本来就有
   `padding-inline: var(--tab-shoulder)`(9)——那不是留白,是「活动 tab 的两只肩永远在条里」的
   结构保证。于是 9 + 10 + 6 = 25,要落到 22 只能有人少 3,而**少的那个只能是内衬**:把条的
   内边距压到 6 会裁掉 3px 的肩(`--strip-face` 比 `--pane-face` 暗一档,那只肩看得见)。
   所以左架子声明 `--strip-lead = 线 − 一只肩 − 半枚图标 = 7`,由
   `LeafStrip.module.css` 的 `.tabs > * > [role='tab']:first-child` 落地,缺省仍是 `--tab-pad-x`。
   第一版按本节字面写成「条的左内边距 = 线 − 半枚图标 − 内衬 = 6」,`gate:sessions` ① 当场红
   (图标中心 25),是门把它判出来的。

`gate:sessions` ① 因此从三条量到七条(判据两处同源 / 图标中心 **样本下限 = 3 + 行数** /
一条文字线(会话标题 ∪ 导航行的字)/ 檐上第一格 tab 的图标中心 / 列表左右内边距相等 /
最左一笔墨 / 结构盒最左);`gate:squeeze` ④ 从「起笔线恰好两种」改判「恰好一条」。
读数:64/64,图标 22 枚中心全 22、标题起笔线 `[38]`、檐上图标中心 22、内边距 左 6 / 右 6;
240 档标题占比 81.6%(报障时 35%);`gate:perf` 场景① 54ms(对照 56ms,预算 100,零长帧)。
