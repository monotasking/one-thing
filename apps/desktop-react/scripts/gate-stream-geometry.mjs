...
const TRANSITIONAL = {
  /*
   * ── `tailSlotShiftPx: 1.5` 这一行**已经退场**(G 线 P2-a,2026-09-21)────────
   *
   * 它从 P1 起挂在这儿,判词写的是「`content/tail-snap.ts` 的
   * `TAIL_SNAP_REACQUIRE_DEVICE_PX = 3`(dpr 2 上 1.5px)是尾巴亚像素对齐自己的
   * 地板」,退场判据是「tail-snap 的重认门槛收窄,或者 P2 的锚定器接管亚像素落点」。
   *
   * **那句判词在 P1h 就成了一句假话**:`content/tail-snap.ts` 连同 `snapTail`
   * 整件删除(正本 §12.6),于是这一行指着一个不存在的文件,退场判据谁都判不了 ——
   * 仓根第二条立法说的正是这种「靠注释证明」的账(勘察记录在正本 §13.1.4 ⑤)。
   *
   * **删它是量出来的,不是推的**:P2-a 搬迁前的基线那一趟(dev,16 个场景全跑),
   * `pinned.maxPx` / `pinned.stepPx` **十六格全是 0**(`pinned.moves` / `flips`
   * 同样全 0)—— 被 snap 的那一件没了,那一下位移也就没了,`BUDGET` 的 1 自己就够。
   */
  /**
  ...
  /**
   * ⑬ **只在超量那一档的纯文本那一格**:座位归零那一帧屏上还剩 **4.5px**
   * (`dSh: -14` —— `scrollHeight` 缩了 14,屏上跟着动了 4.5)。
   *
   * G 线 P2-c 的交接帧合写**治得了它**,但治它的代价是另一道门当场红:真店档上
   * 送出去那一刻列还在补历史(尾窗 10k → 全列 112k),视口离底几万像素,而这一支
   * 一写就把那 102,277px 一次走完 —— `gate:send-flow` 超量 ① 从 1 段变 2 段
   * (A/B 2/2 红,拆掉 2/2 绿;读数在正本 §19.5)。所以那一支收了一条前提:
   * **视口与列底之间只剩垫块那一截时才算交接**(`m.gap <= written + EPS`),
   * 而 `big:text` 那一下恰好不满足它 —— 补历史与座位交接撞在同一帧。
   *
   * **十四格里只有这一格达不到**:七个短会话档与其余六个超量档都是 0px。
   * **退场判据**:真店档上「列补历史」与「座位交接」不再同帧(按屏进那条链把
   * 补历史挪出交接窗,或者垫块在补历史期间不退役)—— 那一天这一行删掉。
   */
  'big:text:seatHandoverStepPx': 4.5,
  /*
