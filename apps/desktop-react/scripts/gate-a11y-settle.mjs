 ...
 *
 * **那笔留账 09-02 已结清**:Dock 仍然挂在 `<main>` 外面(它是浮在整扇窗上的一条),
 * 但它的宿主容器从 `<div>` 换成了 `<nav aria-label>`(`AppShell.tsx`),于是瓦与名字条
 * 都落在一块地标里了 —— 对**真的用鼠标的人**也不再有那条 region 违例。
 *
 * **停车这一手仍然保留**:它防的是物理光标(窗口正好开在人的鼠标底下,Chromium
 * 给底下那块瓦补一次 mouseenter),与地标是两件事 —— 那条名字条本来就不是这道门
 * 想扫的东西,让每一趟从同一个惰性点起跑,读数才可比。
 * ──────────────────────────────────────────────────────────────────────
 */
const PARK = { x: 2, y: 2 }
