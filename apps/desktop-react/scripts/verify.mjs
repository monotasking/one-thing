 ...
 * 换世界时那块面两头都在场、切换那一拍最长帧在第五轴预算内 —— 09-12 起,「同一个
 * DOM 节点」那句随 W4 把瓦搬进按空间隔离的拼贴树而作废,判词在 gate-workspace.mjs
 * 文件头 ⑤ 第三次口径更正),全是确定的答案,不看机器状况、没有余量。
...
/*
 * ── gate:sessions(会话侧栏 A1,09-12;正本 `docs/sessions-sidebar-2026-09.md` §6)──
 *
 * 与 gate:layout **同一条理由、同一个价位**进得来:它断言的是**几何与命中测试**,
 * 同一份代码同一个视口跑一百遍是同一个答案 ——
 *  · 「所有元素的左缘对着红灯中心,没有一个越过它」是两个矩形之差(拍板 2);
 *  · 「标题净宽占行宽 ≥ 60%」是一个比值(报障读数 83px / 35%);
 *  · 「悬停那颗 ⋯ 真点得到」判的是 `elementFromPoint` —— 那次报障里钮一直在
 *    DOM 里、opacity 也是 1,盖在它上面的是一块 `opacity: 0` 的时间 span
 *    (独立层叠上下文 + DOM 顺序在后):眼睛看得见、鼠标点不到,而
 *    **只有命中测试说得出这件事**,typecheck / lint / 单测一个都看不见。
 *
 * 它自己那一步 ⑧ 是**读数**(24 条会话的冷开毫秒),判据仍是
 * `src/perf-budget.ts` 的那两格;400 行那一档归 gate:perf 场景①,两道门各量
 * 自己那一档 —— 这一格与「gate:perf 不进 verify」那条裁定不冲突:它量的是
 * 一个有一个数量级余量的小夹具,不是那道八场景的重门。
 */
run('gate:sessions', 'npm', ['run', '--silent', 'gate:sessions'])
/*
