 ...
 *     (裁决 6 的两端)。
 *  ④ **侧栏形**(≤760)问的是另一件事:**标题的左缘只有一种**(09-13 对齐律
 *     §8 第 3 条「一条文字线」)——每一行都有 16px 图标列,普通聊天也画。
 *
 * ── ③ 与 ④ 为什么必须分档问(09-12 立,09-13 按 §8 改口)──────────────────
 * 09-12 拍板把时间列在侧栏形里整格降掉(病历:240px 的架子里标题净宽只有
 * 83px / 35%),同批还把字形改成「只在有字形的行上画」。**后半件在 09-13 被
 * 对齐律 §8 翻了回来**:每一行都有那一格,整块面只有一条文字线。于是:
 *  · 左缘那条竖线照旧**量标题而不是量字形盒** —— 不是因为字形盒会缺席(它今天
 *    不会了),而是因为标题才是人看见的那条起笔线;两档都恒是一条线。
 *  · 「时间列右缘逐行对齐」在侧栏形里量的是一个 `display: none` 的盒子(矩形恒零),
 *    它会**恒绿** —— 那比红更糟(一道说谎的门)。
 * 所以那一条留在它成立的那一档,侧栏形换上 ④ —— 起笔线**恰好一条**,
 * 出现第二条就说明 padding / 间距 / 字形列在某些行上漂了。
 *
...
async function checkRowGeometry(page, band) {
  return page.evaluate((width) => {
    ...
    for (const row of rows) {
      ...
      /*
       * 字形盒是行的**第一个元素子节点**。09-12 时它只在带字形的行上在场,所以
       * 这里判的是「它不是那只标题 span」;**09-13 对齐律 §8 第 2 条之后每一行
       * 都有它**(普通聊天画淡灰气泡),那句判据因此恒真 —— 留着不动是对的:
       * 它照旧只进读数,「图标中心 = 那条线」由 `gate:sessions` ① 量(这道门量的
       * 是挤压,不是对齐)。
       */
    ...
    if (wide) {
      ...
      if (spread(titleLefts) > 0.5) {
        problems.push(
          `总览形:标题左缘没对齐,散布 ${spread(titleLefts).toFixed(2)}px`
            + '(每一行都有字形列,起笔线该是一条 —— 09-13 之前这一档靠'
            + ' `.title:first-child` 那格外边距补,那条规则已随对齐律 §8 退役)',
        )
      }
      ...
      /*
       * ④ 侧栏形:标题的左缘**只许一种**(09-13 对齐律 §8 第 3 条「一条文字线」)。
       *
       * 09-12 到 09-13 之间这条断言写的是「恰好两种」——那时字形是条件渲染的,
       * 没字形的行从盒子左内缘起笔、有字形的行让 20px,两条起笔线是**设计**。
       * §8 定稿把它翻了过来:每一行都有 16px 图标列,所以整块面只有一条文字线。
       * 多于一种 = 某些行的 padding / 间距 / 字形列漂了,那正是这条要抓的。
       */
      ...
      if (kinds.length > 1) {
        problems.push(
          `侧栏形:标题起笔线有 ${kinds.length} 种 [${kinds.join(', ')}] ——`
            + ' §8 第 3 条只许一条(每一行都有字形列,普通聊天也画)',
        )
      }
    }
    ...
    try {
      ...
      for (const band of CONTAINER_BANDS) {
        ...
        if (geom.problems.length) {
          ...
          console.log(
            `  ✓ 容器 ${band}px:标题只截断 · ${band > 760 ? '两列各自一条竖线' : '起笔线恰好一条'} · 树零横向溢出`,
          )
        }
      }
    } finally {
