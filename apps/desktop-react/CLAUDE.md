...
- **Dock 两种形,判据是「有没有自己的一条地」(09-13 用户拍板,样例页 `docs/dock-always-bar-proposal-2026-09-13.html`)**:**常驻 = 主区已经朝那条边让出一条带,那条带就是 Dock 自己,所以它贴满整条边(整边浮栏);自动隐藏 = 盖在内容上的浮层,所以它是一枚药丸。四条边同一个形**——横边一条横栏、竖边一条竖栏,皮(玻璃 `--glass` + `--dock-blur` + `--line-1` + `--sh-2` + `--r-dock`)与瓦的大小 / 缝 / 内距 / 放大**一个字不改**,变的只有「有多长」与「瓦在栏里靠哪头」(`dockAlign` 在栏形下落成 `justify-content`,不再是整条的定位)。起因是用户报「常显时让出来的那条带在不同页(终端黑底 / diff / 浏览器)底下露出不同颜色」——病根是 09-01 的**内衬形**让位:带子的颜色是**底下那块面**的,一页一个样。①**让位四条边统一成平移形**,落点 `.main` 的四向 padding(打在 `.shell` 上会把架子整体推走、露出外壳底色,那是 7823cff9 修掉的异色带;打在 `.center` / `[data-panel-layer]` 上是内衬形,本条推翻它,`.composerDock` 那条 `bottom` 覆写随之删除——`.center` 整格已经在让位线以上,留着就是让位算两遍)。②**形与让位是同一个事实**(`dockDisplay === 'always'` ⇔ `data-dock-reserve` 在),两处读法一个判据:壳按属性钉栏的两头(`--dock-align-*` 归零覆盖沿边那两条 50% 定位),Dock 按 store 挂 `.bar` + `data-dock-form`。让了位却画药丸 = 带里露出别人的底色;没让位却画满边栏 = 栏盖住内容。③**让位量 = 栏厚 + 两侧各 `--sp-3`**,`--dock-thick = 瓦 + --dock-pad-y × 2 + --bw-1 × 2`——**厚度这一维恒吃 `--dock-pad-y`**(横排钉高、竖排钉宽,两条式子写的都是它;`--dock-pad-x` 是「长的那一维」的内距),09-13 之前 `--dock-reserve-v` 拿 `--dock-pad-x` 且两条都漏了边框,真机量到栏外留白 16 / 11 而不是 12。真机读数(1280×860 / md 档):四条边栏外留白**各 12.00**、栏厚 62、让位 86、栏长 1256(横)/ 836(竖),放大到顶时瓦越过栏边 6.4px 而底板逐字不动。④**顶栏是这扇窗的顶带,Dock 的栏形永远从顶带之下起(09-13 用户裁定)**:上 / 左 / 右三条栏的上端是 `calc(var(--topbar-h) + var(--sp-3))`(= 56),不是 `--sp-3`;底边不动。**「让位把顶栏一起推下去」不成立**——红绿灯与顶栏同一行、必须从 y=0 起(09-01 裁定),所以只能让栏自己从顶带之下开始。病历是本单真机量出来的(CDP `elementFromPoint` 3×3 采样):栏从 y=12 起时 `top` 档整条栏压在顶栏那一行上(顶栏 y 0–44,栏 y 12–74),标签「New session」与 Agent chip **各 9/9 采样点被盖**,`right` 档 Agent chip **3/9**;`left` 档功能上不盖谁,但栏顶顶在 macOS 合成的红绿灯底下(x 8–70 / y 10–30),是视觉事故,同一条判据一起治。改后三档**各 0/9**,栏顶 y = 56.00、栏底留白仍 12。**没有一道门量顶栏**——`gate:squeeze` 的两处预留检查量的是 composer 与侧架子,这一条今天靠判词与这行读数守着。
