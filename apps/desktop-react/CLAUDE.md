...
- **说得出「还有多少」才用 `ui/Progress`**(09-17 立,起因:模型下载 112.8 MB / 冷 191 秒,屏上只有一句「正在下载模型…」被用户点名):它与 Spinner / 骨架**不争位置** —— 那两样说「在忙」,它说「走到哪儿了」。说不出分子分母的等待仍然是 Spinner。`value` **缺席 = 不知道,不是 0**:画一条停在 0% 的槽会被读成「开始了但一个字节没走」,所以缺席一路缺到 `aria-valuenow`(不报那一格,读屏念「忙」)。屏上那句「43 MB / 113 MB · 38%」归调用方画,件只画槽。
...
- **输入框属于会话叶,外壳不认识它**(W5-c,2026-09-13 用户拍路线 A;正本
  `docs/composer-in-leaf-2026-09.md`)。起因是报障「composer 在其他 tab 页也存在,导致会遮挡内容」——
  病根是它挂在外壳的 `.center` 上,显不显示与那一格装的是什么内容无关。**新内容要一条自己的底栏,
  就在它自己那一种里渲染**(`content/kinds/<kind>.tsx`),外壳零改动;跟着它的状态(草稿 / 面板状态 /
  读数订阅)一律**按 `sessionId` 分家**,模块级单例那一格是「全应用只有一块」这句话的化石。
  两条顺带的法:①绝对定位那一格的让位只在「谁画到窗底」的那一层写一次,搬进叶之后再写就是让两遍
  (真机差 44px);②`focusIntoRef` 点名 `owner` 是**收窄**,点不到要退回不点名 —— 不然没报 owner 的
  那几种作用域会静默哑掉。
...
- **Dock 只有一个形:一枚独立悬浮的玻璃药丸;两档的差别只有「平时藏不藏」(09-25 用户拍板,推翻 08-31 起的让位)**:「永久展示不再排开其他元素,只是自动展示模式下的展示即可」—— **常驻 = 自动隐藏档唤出来的那一刻永远停在那儿**,压在内容之上,`.main` 不吃任何因 Dock 而起的 padding,外壳不挂 `data-dock-reserve`(属性、`.reserveSm/Lg`、`--dock-thick` / `--dock-reserve-*` 式子、常驻档那套「跨整条让位带的 flex 容器」、`dock-reserve-css.test.ts`、`gate:squeeze` 的 [4/11][5/11] 两步全删)。条的长度**恒为瓦之和**,皮(`--glass` + `--dock-blur` + `--line-1` + `--sh-2` + `--r-dock`)两档逐字相同,`Dock.tsx` 不订阅 `dockDisplay`;**Dock 只占自己那点地方,不改任何人的 background**(没有 `--dock-ground`)。**贴边距 `--dock-edge-gap` = 4**(09-25「视觉上应该更近」,原 `--sp-3` 12);**顶栏是这扇窗的顶带,条从顶带之下起**(09-13 裁定保留):停上边 `top: calc(--topbar-h + --dock-edge-gap)`(`.hidden` 推出去时把顶栏那截一起算上)、停左 / 右边靠前一档 `top: calc(--topbar-h + --sp-3)`;自动隐藏的停稳位由宿主**现读条自己的定位值**(`getComputedStyle(dock)[edge]`),不读 token。**「在坞上」是一个盒子,不是「条 ∪ 脚下是瓦」**(09-25「不管在哪个方向,从应用边到图标距离边最远的地方时,icon 的大小应该不变」「距离边体验上无缝」):条里一块透明感应面 `[data-dock="hit"]`(`Dock.module.css` 的 `.hit`,排在瓦之前、瓦盖在它上面),外侧伸满 `--dock-edge-gap` 到窗边,镜头开着时内侧伸到放大后瓦的最远处(`瓦 × (峰值 − 1) − --dock-pad-y`)、主轴跟底板一起长;`useDockLens` 的交叉轴判据读它的活矩形 ∪ `onTile`。进门用静息那一格(要碰到条才亮),留驻用开着那一格 —— 于是交叉轴上只有「窗边 → 瓦顶」一个区间,区间里瓦的大小只随主轴变,瓦与瓦之间探出条外那截缝不再熄镜头。`gate:dock` ⑩ 改量「padding 0 / 条外侧到边 4 / 感应面外侧落在边上」。
