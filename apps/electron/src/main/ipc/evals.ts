...
import { createIncidentForTurn } from "@onething/backend/wiring/evals/incident.js";
...
//
// 本体已随结构债 P4c 第四批搬进 `@onething/backend/wiring/evals/incident.ts` ——
// 另一个调用方(命令总线上的「重试 = 迟到负信号」)在装配层,够不着 `@main`。
// 这里保留再导出,老的 import 路径一字不改。
export { createIncidentForTurn };
