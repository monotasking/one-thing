...
> **2026-08-23 第三轮拍板**:#13 开(OpenAI 直连原生出图,两闸:引擎 requestedOutputModalities 对 servedBy in-loop 通用化、能力表“原生出图工具”事实优先于目录输出模态)、#14 改(OpenAI 档位/默认/detail original 按官方模型页)→ **P4-9 已落地**。
> **P4-9 落地记录(#13 + #14)**:
> - **#13 闸一(账本)**:`model-capability.ts` 新增 `OPENAI_IMAGE_TOOL_MODELS`(官方 `/docs/guides/tools-image-generation` 的 Supported models 十条逐名比对,容忍 `vendor/` 前缀与 `-YYYY-MM-DD` 快照后缀;`gpt-image-*` / `gpt-5.4`(非 mini/nano)/ `gpt-5-mini` / `o3-mini` / `gpt-5.6` 都不在表里)。它在 `resolveCapability('imageOutput')` 里**排在 registry 之前、override 之后**(与 codex `nativeTools` 捷径同一地位——目录说的 `output_modalities:['text']` 讲的是*模型*的输出模态,图是**工具**产出的),`source:'pattern'`;`resolveImageOutputServedBy` 里 openai + 命中 ⇒ `'in-loop'`,排在 `imageOutput` 裁定之后(用户 override `false` 一票否决整条闸)。`gpt-image-*` 仍 `'dedicated-api'`。
> - **#13 闸二(引擎)**:`runtime/providers/model-registry.ts` 新增 `onethingModelServesImageOutputInLoop`(与 `onethingModelSupportsImageGeneration` 互斥的另一半:那支答“换不换通路”,这支答“图在不在回合里出”),backend 侧 `wiring/providers/model-registry.ts` 再包一层 `modelServesImageOutputInLoop`;`stream-executor.ts` 的 `resolveRequestedOutputModalities` 改成两段:codex 原生工具表(逐字不变,含 OAuth 判据 —— **命中即返回,providerId 是 codex 而没命中就到此为止**,否则 API-key 的 codex 会因目录条目上的 `nativeTools` 悄悄拿到图)→ 其余家问账本的 `servedBy === 'in-loop'`,并同受 `enableToolCalls ∧ supportsTools` 闸(原生出图是工具表里的一项)。OpenRouter / Gemini 的 in-loop 模型也会拿到 `['image']`,**它们的方言不读这一格**(按 profile 自己决定发 `modalities` / `responseModalities`),无害;core 的 `assertAgentOutputModalitiesSupportedByCapabilities` 也过得去,因为账本已经答出 `imageOutput:true` ⇒ `ModelProfile.toAgentModelCapabilities` 会补上 `image-output`。
> - **#14 档位**:openai 的 `openAIProfile` 改为按 gpt-5 小版本给表 —— o 系列 `low/medium/high`;gpt-5(5.0,含 -mini/-nano)`minimal/low/medium/high`;5.1–5.3 `none/low/medium/high`(**`minimal` 从 5.1 起消失**);5.4/5.5 `+xhigh`;5.6+ `+max`。“5.4 默认 none”记在 `defaultOn:false`(`defaultEffort` 的类型排除 `'none'` —— 那不是一档强度);其余默认沿用 medium 并注明“未核”。`clampOpenAIReasoningEffort(effort, efforts?)` 改按账本的 per-model 表钳:在表里原样发,不在就沿强度阶梯**先向下**(`max→xhigh→high`)、向下没有再向上(`minimal ⇒ low`);账本没话说(自定义端点 / copilot)退回换法前的四档表,字节不变。两条线(chat 的 `reasoning_effort`、Responses 的 `reasoning.effort`)共用这一支。
> - **#14 detail**:`OPENAI_RESPONSES_IMAGE_DETAIL_VALUES_WITH_ORIGINAL` 只给 openai 一家,且**按模型开** —— `OpenAIResponsesProviderOptionSupport.imageDetail` 多一个函数档 `(turn) => 值域`,判据是账本的 `onethingOpenAIAcceptsOriginalImageDetail`(gpt-5.4+);xAI 两条通路仍是标准三值。
> - **fixture diff 只有一份**:`responses/openai/tools-named-thinking-max.request.json` 的 `reasoning.effort` `high → xhigh`(gpt-5.5 + effort max)。四套快照其余零字节变。
> - `@shared/ipc` 的 `ThinkingEffort` 与 ThinkToggle 的 `X High` / `Max` 标签**本来就有**,无需补;ThinkToggle 仍过滤 `'none'`。
> **2026-08-23 第二轮拍板**:#12 选 A(文件输入只在 codec 能投递时声明);Responses 通路切(先 Grok 后 OpenAI);Gemini 图像模型回普通流 + 多轮改图(assistant 回放生成图经媒体读端口);citations 最小展示做;智谱/Kimi/Qwen “先上传再引用”文件通道先研究再做。执行:P4-1(#12)∥ P4-2(Gemini)→ P4-3(citations UI)∥ P4-4(Responses-Grok)→ P4-5(Responses-OpenAI)→ P4-6(上传通道)→ P4-7(UI 侧“文件能力”诚实:renderer `useActiveModelCapabilities.supportsFiles` 今天是 vision 的别名,需一条把“账本 fileInput ∧ 线路 transport”暴露给渲染层的通道)。P4-4 已落地(1491a19b,与 P4-1/P4-2 合并提交):`OpenAIResponsesWire` 通用化——codex 专属项(`/codex/responses` 归一化、OAuth 六头与 401 刷新、store:false、image_generation 原生工具、include 与 reasoning 同生共死、effort 钳位、fallback instructions、providerDataTag、错误文案、usage 抽取)全部下沉为 `ResponsesDialectSpec` 字段,`CODEX_DIALECT_SPEC` 一份常量共用于配方与门面;Grok 两 id 切 `/v1/responses`(`GROK_DIALECT_SPEC`:Bearer、reasoning{effort} 线型 id 仍 grok-effort、include encrypted 恒带、store:false、usage 三桶 + cost_in_usd_ticks、transport 增 file-input、providerOptions imageDetail+searchParameters(官方 /v1/responses 同形对象 1:1 迁)、url_citation annotations → provider-data citations);codex 9 份快照零字节变 = “非 codex 冒烟”门过;官方两页对 reasoning.effort 取值矛盾(REST 参考 vs 能力页),按能力页、disabled 不发,待真机;内置工具形状核到但故意不接。P4-5 + P4-8 已落地(6fac8591):openai 切 `/v1/responses`(`OPENAI_DIALECT_SPEC`:include 恒带、gpt-5.1+ disabled ⇒ effort none、text.verbosity、input_image.detail、image_generation 钩子已备但闸门未开 → #13;custom-openai/copilot 仍 chat);gemini 名字兜底与专用流 gemini 分支退役;落库 mediaType 随真实类型;UI 文件能力经 `models.getModelCapabilities` rpc 问 provider(投影归 runtime model-query-presentation)。
P4-6 已落地:Kimi “先上传再抽取”通道 —— 新策略 `AttachmentChannel { handles(part); prepare(turn) }` 挂在 base `Dialect.attachments`,`OpenAIChatWire.buildBody` 序列化前 `await prepare`(读 `turn.messages`,`request.messages` 本体不动);`KimiFileExtractChannel`:`POST /v1/files`(purpose file-extract)→ `GET /v1/files/{id}/content`(text/plain)→ 按官方在该 user 之前插一条 `system`“File: <name>\n<text>”并移除 file part → `DELETE`(fire-and-forget);失败退回 Undeliverable + warning;回合内按内容 hash 去重;`TurnTransport` 只读回合端口(baseUrl/fetchImpl/headers 闭包)给策略做副请求;`fileViaExtraction` 传输声明让投影绕过账本的模型模态否决(用户 override 仍一票否决);kimi/kimi-code 声明 file;delivery-invariant 对“由通道接管的格”跳过 codec 断言(唯一破例)。后续:跨回合缓存(装配层会话级)、kimi-code 套餐端点是否同 `/files` 待真机、DELETE 在短命进程兜底、pending 轮询按需。
> **真机核对清单(累计,不是门)**:① Codex gpt-5.5 普通问答走文本流、要求画图能出图;② DeepSeek vision-exp 发图 dump 里 `image_url`、不再调 read;③ Kimi Code Plan 请求带 `prompt_cache_key`;④ OpenRouter 图像模型出图是否落到消息(流式 `delta.images` 形状);⑤ 用量面板 OpenRouter “provider quoted” 行;⑥ Gemini 图像模型:流式 inlineData 整块/分片、多轮回放连同 markdown 正文是否被接受;⑦ xAI grok-4.6 `/v1/responses`:effort 取值(两页矛盾)、流事件名 reasoning_summary_text vs reasoning_text、encrypted_content 回放、usage cost_in_usd_ticks、错误头名;⑨ Kimi 上传抽取:真 key 发一份 PDF 看 system 抽取文本、kimi-code 套餐 `…/coding/v1/files` 是否可用;⑧ OpenAI gpt-5.5 `/v1/responses`:`effort:none` 不带 summary 是否被收、`minimal` 对 5.5 是否 400、encrypted 回放、usage cache_write_tokens、tool 结果图(input_image in function_call_output)。P4-2 已落地:Gemini 图像模型 servedBy in-loop、请求 `responseModalities`、流解析 `inlineData` → provider-data、多轮改图 = GeminiPartCodec 在 model 角色回放消息里的生成图(`mediaId:`/`media://` → `ProviderContext.media.readImageBase64`,实现在 backend `wiring/agent-loop/providers/media-reader.ts` 经媒体库,不 import node:fs);基类 `buildBody` 放宽为可 async;待真机核:流式 inlineData 整块/分片、回放连同 markdown 正文是否被接受;P4 收尾待清:生图落库写死 image/png、`onethingModelSupportsImageGeneration` 的 gemini+image 名字兜底与专用流 gemini 分支退役。P4-1 已落地:`fileInput` 独立能力(override → 目录 inputModalities 含 pdf/file → 规则 → 默认 false),投影 = transport.file ∧ 账本(不知道时按 transport),openai-chat 只 openai/openrouter 声明 file,delivery-invariant 6 条 it.fails 归零。
> **上传通道研究结论(08-23)**:智谱 `file_url` 只收公网 URL(不收 base64、未说可用上传 id),file-extract 已标旧,剩独立“文件解析 API”= 文本抽取;Kimi `file-extract` 须 `GET /files/{id}/content` 取文本自己塞 system(官方:放内容不放 id),`ms://` 只给图/视频;Qwen `fileid://` 仅 qwen-long / qwen-doc-turbo。三态:inline-text(Kimi/智谱)/ id-ref(Qwen、Kimi ms://)/ url-ref(智谱,需自托管 → 不可行)。建议排在 Responses/Gemini 之后,先做 Kimi file-extract。
...
13. **(已开,P4-9)OpenAI 直连是否开原生出图**(gpt-5.5/5.4-mini/5.2/5/o3/4.1 官方支持 image_generation 工具;方言钩子已备;需开两闸:stream-executor 对 servedBy in-loop 的任何 provider 计算 requestedOutputModalities、能力表把“有原生出图工具”当独立事实优先于目录的输出模态;徽标随之点亮)。建议开。
14. **(已改,P4-9)OpenAI 思考档位对齐官方模型页**(gpt-5 minimal/low/medium/high;5.4+ xhigh、5.4+ 默认 none;5.5 无 minimal;5.6 +max;detail 值域 +original)。建议改。
