...
13. **OpenAI 直连是否开原生出图**(gpt-5.5/5.4-mini/5.2/5/o3/4.1 官方支持 image_generation 工具;方言钩子已备;需开两闸:stream-executor 对 servedBy in-loop 的任何 provider 计算 requestedOutputModalities、能力表把“有原生出图工具”当独立事实优先于目录的输出模态;徽标随之点亮)。建议开。
14. **OpenAI 思考档位对齐官方模型页**(gpt-5 minimal/low/medium/high;5.4+ xhigh、5.4+ 默认 none;5.5 无 minimal;5.6 +max;detail 值域 +original)。建议改。
