...
## 8. 交互稳定性四律(2026-08-31 立)

用户 08-31 真机连报一批「闪」:勾选一个模型整表闪一下、点 Refresh Catalog 闪一下。
逐处看都是小事,合起来是**同一件事**——屏幕在一次写操作里失去了连续性。所以这里
立的是律,不是几条修法。

**四律**

1. **写操作就地更新,重拉后台对账。** 禁「清空 → 骨架 → 重灌」这一套三拍。
   写出去的那一刻屏幕就该是写完的样子;重拉回来的答案只用来**对账**(不一致才改),
   不用来重画。
2. **重拉期间旧内容保留在屏。** 骨架 / 空态**只许首载**用——那时屏幕上本来就没有
   内容可留。已经有内容了还画骨架,等于告诉用户「你刚才看的东西没了」。
3. **异步动作必有进行中反馈,而且长在发起它的那个控件上。** 钮自己变文字 + 禁用
   (与复制反馈「已复制」同一拍板)。反馈**只**加在被点的那一个上——一次勾选
   把整表都禁掉,是把一个控件的状态说成了整块面的状态。
4. **列表 key 稳定,禁整树重挂。** 行的身份跟着数据的 id 走,不跟着数组下标或
   一次重建出来的新对象身份走。

**每批自证**:凡动到写路径的批,配一条「重拉在飞时旧行仍在 DOM」类断言——
它守的正是律①②合起来那句话,而且拆掉实现必红。

**执行机制:数据层原语,不靠逐处自觉。**

这四条**不是**给每个组件作者的自律条款。08-31 的勘察结论是:今天这批「闪」的病根
在数据层没有把「谁在写、写的是哪一格」表达出来——一个全局的 `saving` 布尔,
和一个不分首载/重拉的 `status` 三态,逼着每一个消费它的组件各自去猜。
猜错的方式只有那么几种,于是同一种闪在不同的面里长了很多遍。

所以四律的执行落在**数据层原语**上(方案另出):写路径带上自己的身份、
重拉与首载分成两件事、乐观值与对账值分开存。原语落地之前,已经落地的手写反馈
(如 `ModelCatalog` 那颗刷新钮)一律在注释里标「临时手写,待 pending 绑定取代」——
标了才知道哪些是欠的,不标就变成了新的存量。

诊断清单(逐处 file:line 定性、按病型归类)随 08-31 那一批汇报交付,是原语方案的输入。
