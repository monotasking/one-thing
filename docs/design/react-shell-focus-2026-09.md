...
| 2 | **指针操作不 `activate()`**:从 Dock **点开**一块面,焦点一步都不动;浮窗指针置顶只改 z 序不动焦点。 | **开着 —— 裁定成立,但它的「今天的行为」这一句 09-04 S2 量出来是错的,须重新拍板** | §7 的原话「指针操作不调(**点击自己落焦**)」。规则 3 说的是「**挪**到哪焦点跟到哪」,而从 Dock 点开一块面是「**开**」不是「挪」。**09-04 S2 真机读数推翻了括号里那半句**:这条留账原先写的是「点击后焦点留在瓦上」,而 macOS 上的 Chromium **点 `<button>` 根本不落焦**(平台惯例),`registry.onPointerDown` 又因为 `target.closest('button')` 命中而主动让开(注释原话「点在一个自己能接焦点的东西上 → 浏览器自己会落焦,不插手」)—— 于是真实行为是**焦点纹丝不动地留在按之前那个地方**。隔离 store 逐块面量(workspace / providers / files / settings 四块,焦点先摆在输入框):四块面全部开出来了、`activeElement` 全部仍是 `composer-input`、活动路径全程 `[root, composer]`。这正是用户 09-04 报的那一句「触发一块面,焦点为什么到了 inputbox」的机器读数。**要不要改仍是用户的拍点**(两个选项:①点瓦也跟,与键盘那条路合一;②点瓦后焦点落在**瓦自己**上,即兑现「点击自己落焦」那句原话);S2 没有替用户拍,只把事实更正到这里。键盘那条路(`summonItem`)本身**没有病** —— 同一次量把 12 块面 × 四态 × 浮窗/钉边两种落点全走了一遍,焦点每一次都跟过去了。 |
...
| 已打开但看不见:架子上非当前 tab / 架子收成细梁 / 浮窗被压在下面 | 露出来(设 activeId / 展开架子 / `focusFloat` z 序),**位置不变**;跟焦由 `focus-follow` 既有判据接(切 tab / 程序置顶两档已在),缺的档补 |
| 已打开但**被盖层 / 舞台压着**(`blocked`) | **什么都不做**(09-04 S2 用户已裁,见下面那段;此前是「待裁」)|
| 看得见、焦点不在它里面 | `focusTree.activateScope(层, { owner: itemId })`,形态零变化 |
| 看得见、焦点在它里面 | **(b) 隐藏**(09-03 用户改判,推翻同日上午的 (a)),**隐藏的对象按形态定**(09-03 用户追加):钉边 → `toggleShelfCollapsed(side)` **收起整条架子**(面留作当前 tab,与 reveal 的 shelf-expand 成一对;绝不是关掉这块面让焦点掉到隔壁 tab 上);舞台 / 浮窗 / 盖 → `closeToDock`(它们没有「收起」档)。焦点一律由结构归还(§4.5 returnTo)自然回到按键之前的地方,不手动搬。VS Code 终端 ⌃` 一族;命令名回到「切换」 |

**被压着那一形(`blocked`)= 什么都不做(09-04 S2 用户裁定,此前留在「待裁」)**。
理由一句:越过 = 替用户关掉别人。展开说是两条路都走不通 —— 走「露出来」得先把盖 / 舞台
收掉(那正是 §14 明令不做的),走「只聚焦」是把键盘送进一块用户此刻看不见的面里
(焦点环画在盖底下,敲键盘不知道字去了哪儿,比按了没反应更坏)。所以它是第五种动作:
一个诚实的空动作,带着「谁压着」那格读数;想用那块面的人先按 Esc 退掉最上面那一层
(退层链本来就是干这个的),再召唤。判词与代码在 `stage/summon.ts` 的文件头。

**分工**:鼠标点 Dock 瓦仍是 `clickDockIcon`(开 / 收;09-03 与拍点 2 同批裁定
「指针操作不 activate」);键盘命令走 summon。旧那口纯开关(`store.toggleItem` /
`transitions.togglePlacement`)在 S1 把键盘那条路改走召唤之后就零消费者了,
09-04 S2 用户裁定删掉 —— 一个没人叫的第二种打开语义留着只会被下一个人叫起来。
命令 id `toggle:<面>` **不改**(改了会作废用户已存的改绑),只换落点;文案维持「切换 / Toggle」(第四格是隐藏,名字是诚实的;S1 一度改成「召唤」,S1b 撤回)。
