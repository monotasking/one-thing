...
- **工作组（work group）** —— 顶部思考（`message.reasoning`，由 MessageItem 经
  ...
  **自动展开一次为限**（2026-08-19 录屏修）：`Working` 这一票在一个回合里会来回翻
  （工具在跑真 → 答案流出假 → 又调一轮工具 / 又开始思考真），头上的文案跟着翻，
  但框子自动收起之后就闩上，后续轮次不再自动掀开 —— 否则一条几百 px 的 rail 一开
  一合，贴底的消息列表整屏上下弹，用户读到一半的字被卷回组里；七轮的回合弹七次。
  闩上之后进度仍由头上的「Working · 47s · bash ×5」实时报，用户点开即 intent，
  一锤定音。
