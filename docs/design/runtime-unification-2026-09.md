...
| ④ 退役 | **4a**(建设性):CLI 搬到 `apps/cli`、React 壳长出 web 模式接管浏览器壳、新门 `gate:web-shell` / **4b**(破坏性):删 apps/electron、packages/renderer、apps/web、electron-vite、macos_panel、66 个 Vue 专属依赖、`@renderer`/`@main`/`@preload`/`@` 别名、Vue 的 ui:gate 与两道真机门、server `?token=` 口;边界检查器摘 62 条守 Vue 宿主的检查并加 `checkVueHostStaysRetired` | 4a **入库 9ae76227**;4b **入库**(本文 §2.2;用户拍:浏览器壳由 React、macos_panel 去掉) |
...
### 2.3 4b 已做(2026-09-04,编排者自做)

用户两拍:浏览器壳由 React 壳提供;`macos_panel` 去掉。删除台账(每项 `rg` 证零真 import 后删):`apps/electron/`、`packages/renderer/`、`apps/web/`、根 `index.html`、`electron.vite.config.ts`、`electron-builder.vue.yml`、`tsconfig.web.json`、`onething.aliases.ts`、`scripts/{dev-with-logging,dev-self,self-pull,gate-vue-host,gate-web-shell(Vue),ui-style-check,ui-gate,streaming-scroll-harness,take-screenshots,build-macos-panel}.mjs`、`native/macos-panel/`、`resources/native/`、`docs/audit/ui-baseline-2026-08-13.txt`;依赖 66 个(vue / pinia / electron-vite / @vitejs/plugin-vue / vue-tsc / eslint-plugin-vue / @vue/test-utils / lucide-vue-next / @tiptap×10 / tiptap-markdown / prosemirror×3 / @codemirror×15 / @lezer/highlight / monaco-editor / highlight.js / markdown-it×3 / mathjax / @xterm×4 / @fontsource-variable×4 / lxgw×2 / @picovoice×2 / @ricky0123/vad-web / @openrouter/sdk / openai / qrcode / electron-store / eventemitter3 / lunar-javascript / @pierre/diffs / @types/{markdown-it,qrcode}),`@types/uuid` `@types/diff` 留(库仍在用);`scripts/gate-web-shell-react.mjs` 改名接管 `gate:web-shell`;`web:dev` / `web:build` 指 React web 模式;`dev-unified` web 泳道固定 React;`lib/dev-self.mjs` 泳道探测只认 React 入口(dev-self 泳道退役);`vitest.config` 去 vue 插件与三条别名;`eslint.config` 去 vue 插件与两个分区;`tsconfig.json` paths 只剩 `@shared`;`tsconfig.node.json` 去 electron.vite / apps/electron / apps/web;`electron-builder.yml` 去 resources/native 与字体排除;`build-desktop.mjs` 去原生面板段;`gate-native-abi` 去 macos_panel 目标;`log-check` 与 `transport-gate`(含基线)去 Vue 文件;server `http.ts` 删 `acceptsQueryToken` 与 `?token=` 分支(测试改为断言 401);边界检查器从 HEAD 恢复后只摘 `checkElectronHost*` + 9 条明确扫 Vue 树的 + 真跑红的 10 条「runtime 拥有 X」族(它们读 apps/electron 文件),共 62 条,新增 `checkVueHostStaysRetired`(目录 / 依赖 / .vue 文件三判据,反证目录回来与 vue 依赖回来皆红),`checkShellsDoNotImportOtherShells` 去掉 renderer 那半;核心策略表 `ephemeral-policy.ts` 三条指向 Vue 测试的证明改指装配层 status.test 与 background-status.test;`architecture-boundaries.test` 三例改口径;`status.test` 去 renderer/voice.ts;删两个只为 Vue 存在的后端测试(window-target、session-events-listraw-projection)与 logging 测试里一个 electron-host 用例;`resourcesPath` 两处按 skills/loader 同手窄化(electron 类型不再进 node typecheck 图);`bound-fetch` undici Response 经 unknown 过桥。CLAUDE.md 重写为单壳现状。
