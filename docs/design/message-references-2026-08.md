...
- 真机反馈三修(同日):① `~/` —— 渲染端仍没有 home,改为**主进程 stat 展开**:`statOnethingPath` 接 `homeDir`(桌面传 `os.homedir()`,server 已按沙箱根展开不传)并在响应里回 `path`(实际 stat 的绝对路径),`open.ts` 对 `~` 开头的引用先 stat 再用回来的 `path`;② 模型写的多是 `summary.md` / `src/app.ts` 这种裸文件名/相对路径 —— 行内代码 autolink 放行裸文件名(须带字母开头扩展名,排除一张短顶级域表;`.js/.ts/.sh` 视为扩展名),`resolveRelative` 改为返回**候选数组**(会话各工作目录根 + 本会话 write/edit/read 碰过且以该相对路径结尾的文件,后写优先),`open.ts` 逐个 stat 取第一个存在的;提示词片段同步写明;③ 浏览器 —— 原先先建 tab 再开面板,BrowserPanel 挂载时 store 仍空会再开一个空 tab 盖住,改为先开面板 + `ensureLoaded()` 再 `openTab(url)` 并显式 `selectTab(id)`(`openTab` 现在返回 tab id)。
- 用户裁定(同日,推翻上一条的 ②-裸文件名部分):**裸文件名(`foo.md`)不作为文件引用** —— 与域名同形(`.md/.py/.ai/.io/.sh/.ts` 全是真实顶级域),宁可不链接;转而在提示词里**强制**模型写绝对路径(`references.md` 明写"裸文件名永远不会成为引用、从聊天里打不开")。带目录的相对路径(`src/foo.ts`)与候选解析(工作目录根 + 本会话工具碰过的文件)保留作为兜底。`main.py:12` 这种"像 scheme"的写法归 null(既不是文件也不是 external,避免 `openExternal('main.py:12')`)。
