...
> **落地记录 2026-09-06**:检索面(壳)十步迁移全部入库,正本
> `apps/desktop-react/docs/search-panel-2026-09.md` §7 十步表逐行带 sha。本文 §9(壳)按落地改写成事实,
> §12 增三条被 09-05 用户裁定推翻的旧裁定(§7.2 不分页 / §9 组头带 total 与查看全部 / §4.5 ⑤ 预览原话),
> §7.2 与 §4.5 ⑤ 就地划掉并指过去。
>
...
| 己 | 全部档 | ~~**(a) 分组总览不混排不分页,每组「查看全部」进单类 tab 翻页**(v3 已认可)~~ → **2026-09-06 改**(§12):不混排照旧,**分页与「查看全部」两句被推翻** —— 每一块自带 `groups[].cursor` 原地续页,组头退役,「查看全部」进行的右键菜单 |
...
**④ 谁来画 / ⑤ 算不出** —— 预览渲染注册表按 kind 取组件,没有的画 Row 放大版;预览窗状态表:empty / loading(120ms 后才画骨架)/ ready / error(**画字典句「预览不可用」,后端原话只进 `getLogger('search.preview')`** —— 2026-09-06 改,见 §12;列表不受影响)/ 超量(截断并标)。**零副作用**是硬规矩:预览不改已读、不改会话打开态、不写最近、不改播放队列、不触发文件 mtime。
...
- ~~每组带 `total`(能力知道就给)和「查看全部」→ 切到该能力 tab,cursor 从头。~~
  **2026-09-06 推翻**(§12):组头整个退役,`total` 随块尾那条读数「共 N 条」说,
  「查看全部」搬进行的右键菜单「只看这一类」。
- **不跨类混排**:各路 `score` 不可比(BM25 与静态打分与文件名匹配不是一个量纲);混排出来的次序说不出理由。要混排就得给每路做置信度换算(C 方案),不推荐。**这一条没变。**
- ~~**不分页**:总览的目的是「大概在哪一类」;要翻页去单类。~~
  **2026-09-06 推翻**(§12):每一块自带 `groups[].cursor`,原地续页;不混排照旧。
...
> **本节是事实,不是计划**(2026-09-06 按落地改写)。检索面的**正本**是
> `apps/desktop-react/docs/search-panel-2026-09.md`(规矩 R1–R12、三张状态表、分页状态机、
> 十步迁移与每步落地 sha);这里只留「这块壳与本方案的接口」那几句。09-05 用户看真机连报
> 「组头多余 / 零命中占行 / Create prompt 冒充结果 / 预览标题两遍 / Load more 跳回顶部」,
> 那一轮的裁定推翻了本节原来的三句话(见 §12 最后三行),下面写的是**改完之后的样子**。

- tab / 图标 / 过滤片 / 分组次序全部由 `search.capabilities` 回来的 manifest 算出;`all` 固定第一。
  **输入框那句占位也是**(2026-09-06):档名由 `search/capabilities.ts` 的 `scopeNamesOf` 从自述串出来,
  注销一个能力它自己就少一个名字 —— 从前那句写死的「搜文件、章节、消息、会话…」上一次说对是在
  「章节」还是一个档的时候。
- 结果行按 `target.kind` 从目标渲染注册表取组件(`src/search/targets/<kind>.tsx` 一种一文件);缺渲染器画标题行并 dev warn。
- **一张清单,不分组,块之间只有一格空 + 一条发线**:全部档与浏览态是同一只 `SearchList`,
  每块 = 一个能力的命中,次序按自述 `order`,跨块不混排。**没有组头、没有「N in total」组头、
  没有「查看全部」**——「只看这一类」进了行的右键菜单。零命中的能力一块都不占。
- **每块自己原地续页**:块末一条项「加载更多 · 已显示 5 / 41」,按下只把这一块长一页,
  旧行一像素不动、滚动不动、焦点不动(焦点恒在输入框);取尽即换成一条读数「共 N 条」。
  游标来自 `groups[].cursor`(§8 只加的那一格),回传给同词同片的单类请求就是那一块的下一页。
- **动作不是结果**:页级 `actions`(「新建提示词 “jira”」「新建今天的日记」)画在清单末尾分隔线下,
  不计数、不戴徽、不进预览、不参与零结果判据。
- 页脚是一行读数,`' · '` 串起来,每一条各是自己那一个 `[data-readout]`:
  搜索中 / 已放宽(三级三句)/ 索引更新中(剩 n)/ 由 <host> 维护 / 索引不可用 /
  语义召回下载中 · 建向量中(剩 n)/ 「<能力名>没搜成 · 重试」(块级失败唯一诚实的落点)。
- **全部档的空词是浏览态,不是分组总览**(S4b;09-01 用户裁定「所有档空词 = 全部会话列表、看得到总条数、能翻页」):壳在**零词元**时不发 `category: 'all'`(后端的 `'all'` 按 §7.2 是不分页的总览),而是问自述里 `browse: true` 的那些能力,**各一组、每组全量可翻页**;今天只有一个,于是屏幕上是一张平铺的会话列表。去问谁**从 manifest 自述读**,壳里因此没有一个能力 id(`no-capability-literals` 那道闸继续零 id)。有词照旧走分组总览。
- 过滤片:空间(当前 / 全部)、角色、时间、含归档、含推理;一律「键 · 值」两格,过滤片是结构传 `filters`,不拼字符串。
...
- 徽:归档、空间(仅在「全部空间」下画)、语义(来自向量路的行,S7)。
- 预览:檐上画一次标题(带命中高亮),`Body` 不画标题;自述**明说**没有 `preview` 的能力零请求、
  画行的放大版;算不出时画字典句「预览不可用」,后端原话只进 `getLogger('search.preview')`。
- 点消息 → 既有 `locate-message` 落点。
- **一把键 = 一张列表的主语** `[ask, query, filters]`,`limit` / 页码 / cursor 不进键;翻页是对同一格
  `patch` 追加。这一条是「Load more 跳回顶部」的根治,细节与四条不变量在正本附录 B §5。
- 三张状态表(生命周期 / UI 生命状态 / 交互状态)在正本附录 B §2–§4;基础件先行,`ui:consume` 只减不增。
...
| **「全部档不分页」** | 本文 §7.2 末句、§0 拍点己 a | 「总览的目的是『大概在哪一类』;要翻页去单类」 | **2026-09-06 删**。用户 09-05 报「Load more 跳回顶部」时同时裁定「每块自己原地续页」:不混排照旧,但每一块自带 `groups[].cursor`(§8 只加的一格),按下只让这一块长一页。「去单类才翻得了页」把一次「我还想多看两条」变成一次换档,是壳把自己的分页做不动推给了用户 |
| **「组头带 `total` 与『查看全部』」** | 本文 §9(改写前第 4 条)、§7.2 第 2 条 | 分组总览要让人一眼看出「哪一类里多」 | **2026-09-06 删**。组头退役:块之间只有一格空 + 一条发线(一条线不该占一个可聚焦、可数、可被读屏念出来的位置);`total` 随取尽那一刻的块尾读数「共 N 条」说,「查看全部」搬进行的右键菜单「只看这一类」。某组 `error` 时的那句「没搜成」也随组头一起搬到页脚(`[data-readout="block-errors"]`)—— 那是组头最后还在干的活 |
| **「预览算不出画后端原话」** | 本文 §4.5 ⑤「error(原话,列表不受影响)」 | 别把失败吞掉 | **2026-09-06 改**。不吞掉这一条不变,变的是**画什么**:屏上画字典句「预览不可用」,后端原话进 `getLogger('search.preview').warn` 与 `data-preview-error`。用户 09-05 原话是「不要露内部串」——一句 `ENOENT: no such file or directory, open '/Users/…'` 对着用户说不出任何他能做的事 |
...
#### 15.8b 认领:清单缺席、文件却在(2026-09-17 下午的事故)

**用户原话**:「为什么下载了也当做没下载?」

**现场**:13:56 用上一版(翻开关即下载)把模型完整下到了
`~/.onething/models/embeddings/multilingual-e5-small/Xenova/multilingual-e5-small/`
(`config.json` 658B、`tokenizer_config.json` 443B、`tokenizer.json` 17,082,730B、
`onnx/model_quantized.onnx` 118,308,185B;14:36:05 日志里有 `embedding model loaded`)。
15:24 换上 §15.8 这一版(判据 = 下载落定才写的清单)重启,4 秒后
`semantic recall turned itself off` —— 文件齐全、装得上,**只是没有清单**,于是被判成
「未下载」,模型行画「未下载 · 约 113 MB」。

**这是新判据的迁移漏洞,不是它错**:清单这条判据要挡的是「进程被杀留下半截 onnx」
(§15.8 判断 1),而它挡不住的是「这份模型是上一版下的」。

##### 为什么不是「看文件在不在」

因为那正是 §15.8 判断 1 否掉的那条路,而且否掉的理由一个字都没变:`FileCache.put`
**直写最终路径**,没有 `.part`,所以「文件在、大小 > 0」与「下全了」之间没有关系;
而「要哪几个文件」是 `@huggingface/transformers` 的事,在我们这儿抄一份迟早漂开。

##### 为什么是「试装一次」

**装得上,是不联网就能证明这堆文件完整的唯一办法**。ONNX 会话建得出来意味着那份
118MB 的 protobuf 从头到尾是完整的,分词器 JSON 解析得出来意味着那 16MB 没被截断 ——
这比任何一张文件清单都硬,而且它不需要知道「该有哪几个文件」。

于是判据松了**一格**,只松在清单缺席那一形上:

| 盘上 | 判定 | 动作 |
| --- | --- | --- |
| 有清单,字节数对得上 | `ready` | 照旧,一次装载都不多 |
| 没清单,目录空 | `absent` | **不试装**(「从没下过」;⑬a 的零网络有一半是它) |
| 没清单,目录里有东西 | 试装一次 | 装得上 → 照磁盘真实文件补清单、`ready`;装不上 → 维持 `absent`,**文件一个都不删**(下一次「下载」等于续传),日志一条 warn 带原话 |

##### 三条施工上的判断

1. **不加第五个态。** 试装那几秒 `ModelDownloader.status()` 答 `undefined`,`status.model`
   **整格缺席** —— 那与「这台宿主管不了模型」同一种缺席,壳早就有画法(unknown
   「检查中…」,一颗钮都不画,开关行不拦路)。报一句「未下载」再翻成「已下载」是**闪
   一下错话**,比不说话糟。壳只补了一句:`semanticStatusPollMs` 在模型那一格为
   `unknown` 时也每 5s 问一次 —— 缺席有两种意思,其中一种(正在试装)会动,而
   「开关关着 + 正在认领」原本会永远停在「检查中…」(那一页没有第二个重新去问的由头)。
2. **不装第二遍 118 MB。** 认领要装一次、嵌入器自己也要装一次,而这条 Worker 上它们
   是同一份文件。所以 `tryLoad` 由 `worker.ts` **递进来**:开关开着(这条 Worker 有
   嵌入器)递的就是 `embedder.ready()` —— `ready()` 是记过账的,认领与正经装载合成
   一次;开关关着才递嵌入器工厂自述的 `verify`(装完 `dispose()` 把会话还回去,这一路
   只要那句判断)。为此 `transformers-onnx.ts` 的装载门槛从「清单在不在」松成
   「目录里有没有东西」,`allowRemoteModels = false` 一个字没动 —— ⑬a 守的零网络
   照旧,清单则**仍然只由 `ModelDownloader` 写**(唯一写者)。
3. **认领成功时不喊宿主换 Worker。** 那一声(`model-event`)的用处只有「让宿主换一条
   Worker 使模型生效」;而 `inUse()` 为真时认领走的就是这条 Worker 自己的装载,它已经
   生效了,换一条只会把刚装好的扔掉重装。所以那一声只在 `inUse()` 为假时喊(那时
   装配层本来就会因为开关关着而忽略它)。

##### 门与读数

- **单测**(`search/index/__tests__/model-download.test.ts` 六条 + `model-protocol.test.ts`
  一条 + `embedding/__tests__/model-store.test.ts` 一条):装得上就认领并补清单 / 装不上
  不认领且文件还在 / 空目录零试装 / 试装期间整格缺席、落定才有话 / 开关开着只装一次且
  不喊那一声 / 认领在飞时按「下载」以下载为准。
- **`gate:search-index` ⑬e**(常跑):⑬c 取消之后盘上留着三份 json、清单不在,⑬d 关开关
  换出来的 Worker 于是撞上那一形 —— 那台假站的零填 onnx **装不起来**,读数:仍是
  `absent`(0.3s)、文件 3 件不多不少、没有清单、假站计数 4 → 4(**认领一个网络请求都不发**)。
- **`gate:search-index` ⑫b**(可选,真模型):下全之后把清单删掉、把 `network.proxy` 指到
  `http://127.0.0.1:1`(连不上)、换一条 Worker → **0.7s 认领回 `ready`**、清单补了回来;
  再把开关打开,那句零词重叠的改写句照样排第一(名次 0)。判据是那句改写句而不是
  `vector === 'ready'`:文档早就嵌完了,新 Worker 没有待嵌的东西,`vectorState` 会一直
  停在开局的 `'downloading'`——「没活干」与「没装上」在那一格上长得一样。
- **真机只读核对**(不碰真店):把真店的 `models/embeddings`(129MB)拷一份、删掉副本里的
  清单,用 `apps/desktop-react/dist-electron/search-worker.cjs` 起一条 Worker ——
  `status.model` 从**整格缺席**走到 `ready`,开关关着 875ms(走 `verify`)、开关开着
  521ms(走嵌入器那一次装载,且**没有** `model-event` 通知帧),副本里的清单
  4 件 / 135,392,016 字节,与原件逐字节相同。
- **反证**:把构造函数里那句 `beginAdoption` 拆掉 → 上面六条单测里的四条当场红
  (认领、整格缺席、只装一次、按下载作废)。

---

## 16. 2026-09-18:占了多少地方,以及那 100 MB 是什么

**用户原话**(09-17):「我要知道搜索占得空间,不管是现在的 fts5 还是向量库。」

### 16.1 病:161 MB 的库里,100 MB 是陈旧的 FTS5 段

答这句话之前先量了一遍真店(`~/.onething/index/search.v1.sqlite` 的**副本**,原件只读):
11,767 份文档、库文件 161 MB、`-wal` 5.5 MB、模型目录 135,392,016 字节。而 09-17 上午量到
的是 128 MB / 11,700 篇 —— **一天只多 67 篇却涨了 23 MB**,期间换过五六次 Worker。

`dbstat` 按表摊开之后病因一眼可见,并在副本上逐步验过(全部由这一批新加的
`SqliteIndex.storage()` / `maintenanceStats()` 与 `IndexWorkerCore` 的那一趟真代码跑出来):

| 表 / 读数 | 收拾前 | `optimize` 之后 | 再 `VACUUM` 之后 |
| --- | --- | --- | --- |
| `docs_fts_data`(FTS5 的段) | **108,695,552** | **8,429,568** | 8,425,472 |
| `docs` | 19,689,472 | 19,689,472 | 19,644,416 |
| `docs_fts_content` | 18,288,640 | 18,288,640 | 18,247,680 |
| 向量那一族(11 张,`vec_docs_384*`) | 1,646,592 | 1,646,592 | **1,646,592**(不动) |
| **库文件** | **161,361,920** | 164,577,280 | **58,765,312** |
| 空闲页 | 224(0.5%) | 25,558(**64%**) | 0 |
| FTS 段数 | 26 | 1 | 1 |
| 耗时 | — | **636ms** | **290ms** |

**161 MB 里 100 MB 是陈旧的 FTS5 段。** 它们是「整键重折 = 删掉再插一遍」的必然产物:
FTS5 的删除写的是一条反向记录,旧倒排要等段合并才真的消失,而自动合并追不上这个仓的重折
频率(真店那份 `meta.generation` = 497,919)。空闲页(224 页 / 0.9 MB)**不是**病根 —— 它是
`optimize` 之后才出现的结果。

三条顺带量出来的事实,都写进了 `runtime/search/index/storage.ts` 的文件头:

- **次序不能反**:只 `VACUUM` 不 `optimize` = 1445ms、161 → 159 MB(陈旧段此刻还算「活数据」,
  照搬一遍而已)。
- **`VACUUM` 之后必须 `wal_checkpoint(TRUNCATE)`**:WAL 模式下 VACUUM 把整个新库写进 `-wal`
  (副本上主库缩到 58.8 MB 的同时 `-wal` 涨到 59.1 MB)。少这一句,「占用空间」会在收拾完之后
  报得比收拾前还大。
- **安全性是量的不是推的**:副本上 `integrity_check` 答 `ok`、文档数 11,767 不变、
  `vec_docs_384` 的 506 行还在,**同一个查询向量的 KNN 前十条(docId / chunk / distance)
  逐字节相同**;`docs.docId` 是 `INTEGER PRIMARY KEY`,FTS5 与 vec0 的影子表也各有自己的
  `INTEGER PRIMARY KEY` / `AUTOINCREMENT`,所以 VACUUM 不动任何被别处引用的 rowid。

**这一条推翻 §5.4 的「不做 `VACUUM`」**。那句话的理由是「换来的只是磁盘占用,而派生数据
删了即重建」—— 在 3 倍的量级上它不再成立,尤其是我们刚答应把这个数**报在屏幕上**。

### 16.2 面:`search.storage` 一口,归类判据是数据

契约只加一条路由 `search.storage` → `{ lexicalBytes, vectorBytes?, walBytes, modelBytes,
totalBytes, measuredAt, approximate? }`。三条判词:

1. **归类问库,不抄清单**(`storage.ts` 的 `vectorTableFamily`):`sqlite_schema` 里哪几张是
   `vec0` 虚表(按建表语句里的 `USING vec0` 认,不按表名),它们的影子表就是「虚表名 + `_`」
   开头的那些,再加 `sqlite_autoindex_<影子表>_N`。**其余一律归字面**(FTS5 影子表、关系表、
   各索引、空闲页)。于是这个文件里没有一个模型名、没有一个维数 —— 换一档 768 维的嵌入器,
   一个字不用改。
2. **只按名字量向量那一族,字面是减出来的**。`dbstat` 是扫 btree 的:全表 `GROUP BY name`
   在真店上要 **790–1120ms**(那一整段 Worker 什么都答不了);按名字问(`WHERE name = ?`,
   约束下推)只扫那十一张表,**真店实测 `storage()` 冷 48.7ms / 热 0.5ms**。所以不做缓存
   (派工单给的线是 200ms)。
3. **缺席 ≠ 0**:`vectorBytes` 缺席 = 这个库里还没有向量表(开关从没开过),屏幕上画「—」;
   `approximate` = 这台机器的 SQLite 没编进 `dbstat`,向量那一半分不出来全算进了字面。
   库的大小按 `page_count × page_size` 算而不是 `statSync`——WAL 里还压着的页在主库文件上
   看不见,两个坐标系只有 checkpoint 之后才重合。

屏幕上是设置 → 搜索页的第二节「占用空间」:三行 + 合计,右对齐 `tabular-nums`,**WAL 并进
「字面索引」那一行的数**(它是同一个库的预写日志,单列一行会让一个自己会跳的实现细节占一行
注意力)。不轮询 —— 这个数不会自己动,只在模型下载 / 删除 / 翻开关 / 向量那一半刚嵌完之后
重量一次。

### 16.3 收拾:启动后一次,四道界

`IndexWorkerCore` 在**启动校对完成且队列排空**(`drain()`)之后跑一次,判据是
`storage.ts` 的两只纯函数:段数 ≥ 8 → `optimize`;**再量一遍**空闲页 ≥ 10% 且活数据
≤ 256 MB → `VACUUM` + `wal_checkpoint(TRUNCATE)`。四道界:一条 Worker 最多一次、欠的活干完
才收、够不着阈值不收、收不动(活数据超预算)只做 `optimize`。失败(别的进程开着库 →
`SQLITE_BUSY`)只记一条 warn。

**空闲页必须量两遍**:真店那份在 `optimize` 之前空闲页只有 0.5%,之后才是 64% —— 陈旧段在
`optimize` 之前还算「活数据」。拿之前的读数问 VACUUM,答案永远是「不用缩」。

真店副本上走完整条真代码路:161,361,920 → **58,765,312**(−63.6%),段数 26 → 1,空闲页
224 → 0,`-wal` 归零,**文档数 11,767 一条不少**,全程 2.0s(其中 SQL 926ms)。

### 16.4 门与反证

`gate:search-index` 加两小步:**⑭**(主库那一间,从没开过语义召回)三个数 ≥ 0、
`vectorBytes` **整格缺席**、`totalBytes` 恰是四项之和、字面 > 0;**⑭b**(⑧ 那一间,假嵌入器
嵌完之后)`vectorBytes > 0`(实测 1,638,400)且和仍然对得上。单测
`runtime/search/index/__tests__/storage.test.ts` 十六条(归类四条、真库读数四条、判据四条、
那一趟四条)。

**反证**(真跑过):把 `vectorTableFamily` 的判据写反(只认虚表自己、不认影子表)→ 三条单测
当场红,`gate:search-index` ⑭b 读到 `vectorBytes: 0` 红;改回来全绿。

### 16.5 留账

- **收拾那一趟没有可观测的中间态可言**:`node:sqlite` 是同步的,`optimize` 跑着的时候这条
  Worker 连 `status` 都答不了。`IndexStatus.maintaining` 只在两句 SQL 之间那一小段答得出来,
  **没有上线上契约**(屏幕上没有它的位置)。
- **这一节只回答「占了多少」,没有「清理」那颗钮**。收拾今天是后台的、一条 Worker 一次;
  要不要给人一颗手动的钮是下一个问题。
- **`approximate` 今天没有读者**:屏幕上四行照旧(它是给排障与门看的一句实话),`dbstat`
  在这个仓跑得到的每一台机器上都在。
