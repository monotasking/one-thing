...
| 10 | **(新)窗口系通道的典型形态**(todo-plan minimize/zoom/drag 等操作 BrowserWindow 的通道):宿主壳自己的 `shell`/`window` 路由 + sender 上下文 vs 维持手写 | P4 终态前统一成一个宿主壳路由;transport 门区分"域通道"(禁增)与"窗口系"(白名单) | 部分已做(08-23 A1-a:`shell:invoke` 一条通道 + 宿主侧 `shell-registry` + `ShellDispatchContext.callerId`,窗口系八域 21 条迁入(todo-plan-window/search-window/settings-window/window/dialog/media-window/notify/deeplink),web 侧同形 `shell-web` 处理者表;顺带删两条死常量,`SEARCH_QUERY` 判为数据面待迁 backend 域;门仍是**一把数值棘轮**,没有为窗口系开白名单;browser 20 条留 A1-b) |
