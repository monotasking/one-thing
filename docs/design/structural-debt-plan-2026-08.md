...
> - **#21 读码结论(已于 2026-08-22 执行)**:`platformApi.resumeAfterToolConfirm` 渲染层零调用点,且引擎侧 `result.requiresConfirmation=true` 已无生产者(toolkit 重建后审批在工具内阻塞,runner 的 pause 抛不出来)——整条"第七条 invoke"是死路。执行口径:删桌面 invoke 通道(preload / `main/ipc/chat.ts` / `apps/electron/src/ipc/chat.ts`)、删 platform 方法与 web 实现、删 runtime `sessions/tool-confirmation.ts` 的 ForIpc 助手;core 的 `command:resume-after-confirm` + `handleResumeAfterConfirm` 保留(引擎能力,退役另列待拍)。
> - **D2 已落地(terminal 迁 router + 能力位默认关)**:`terminalRouter` 七条(create/list/write/resize/kill/attach/ack)+ `backend/rpc/domains/terminal.ts` + `renderer/platform/terminal-client.ts`;`apps/electron/src/ipc/terminal.ts` 整只删(零推送 → 同 collab/acp/themes 判例;该目录本就没有它的测试,`apps/electron/package.json` exports 也从来没有 `./ipc/terminal` 键 —— 它走 `electronHostAliases` 的 catch-all),`@main/ipc/terminal.ts` 93→47 行只剩**广播注入**与**消费者掉线的 detach 边**(`watchTerminalConsumer`)。`ack` 从单向 `ipcRenderer.send` 变成带一条空回执的 invoke(渲染侧本来就不等它;与 `VOICE_AUDIO_CHUNK` 按 #10 退回单向的差别在频次 —— 那是每秒 10–25 块的稳定 PCM 上行,ack 只在终端真有输出时按 16ms flush 节拍走)。**http 语义:七条一律结构化拒绝** `{success:false, error:'Terminal is available on the desktop host only'}`(`list` 额外带 `terminals: []`),不抛、不读盘、**一次都不求值 `getTerminalService()`** —— 懒单例因此仍然不会在 `server:start` / CLI daemon 上 load node-pty(域测试专钉这一条)。闸落在**服务端**而不是渲染侧客户端(与 music/evals/interaction 的判例不同):桌面自己也挂着同一份 HTTP 面(A 期内嵌 server),能力位挡不住拿到 Bearer 的浏览器;能力位 `terminal` 按拍板保持 web 默认 `false`(`platform/types.ts` / `web.ts` 未动),`/api/capabilities` 如实。**放开 = 去掉那层 http 分叉 + 给 server 接推送广播器,两件事必须一起做**(否则是个只能写不能读的终端)。渲染侧保留旧的「未就绪即静默」语义:`write/resize/ack` 三条 fire-and-forget 补 `.catch(ignoreRpcFailure)`(旧 `platformApi.xxx?.()` 在无终端宿主上也不会 reject),`attach` 的失败从 `response?.success` 检查改为 try/catch 折成同形失败信封,可感知行为逐字不变;渲染层三只镜像类型(`TerminalInfo`/`TerminalAttachResult` 与七条 `ElectronAPI` 声明)删,store 改吃 `@shared/ipc/terminal.js` 的真契约。checker 零改动(`rg terminal` 在 `headless-boundary-check.ts` 上唯一命中的是一条无关的 `const terminal = new Promise` 正则)。transport 收紧:channels **110→103**、bridge 881→864、web 1073→1067、channels.ts 299→292,四壳 **3029**。验收:typecheck 0、boundary:gate ok(0 failed)、transport:gate ok、全量 11241 用例(11232–11233 绿/8 skipped,见下「已知抖动」)、battery 264 runs/0 mismatch、build:check 绿、web:build 绿。**已知抖动(与本批无关)**:满载下每趟全量各有一条**不同**的单点红且单跑皆绿 —— `terminal/service.smoke`(真 zsh 还在写 ZDOTDIR 时 `rmSync` 撞 ENOTEMPTY)、`stores/sessions-delete-cascade`、`server/http.test.ts` 的 retry/edit-and-resend(§P4c 第八批已记过这只文件的 listen/close 竞态);battery 首趟亦曾 RED 2 条(steering 文本落到相邻消息),reset 后连跑两趟 264/0 全绿。
> - **C2 已落地(plugins 整域迁 router + web 写面能力位默认关,#16)**:`pluginsRouter` **十九条 invoke**(list/enable/disable/refresh/commands/executeCommand/request/requestAbort/configGet/configSet/uninstall/footprint/install/update/checkUpdates/lifecycleInfo/readTarball/market/pickFile)+ `backend/rpc/domains/plugins.ts` + `renderer/platform/plugins-client.ts`;`apps/electron/src/ipc/plugins.ts` 与它的 `__tests__/plugins.test.ts` **整只删**(`apps/electron/package.json` 的 `./ipc/plugins` exports 键一并删),`@main/ipc/plugins.ts` 317→87 行只剩**两件要 Electron 本体的事的注入**与**一条推送**。**两条推送留在原地**:`PLUGINS_NOTIFICATION` 连端口都不用(总线全局事件,IPCBridge 扇全窗,从头到尾不经过请求面);`PLUGINS_REQUEST_PROGRESS` 改走 `wiring/plugins/events.ts` 的 `configurePluginRequestProgressBroadcaster`,域把 `RpcDispatchContext.callerId` 原样递过去、桌面据它**定向回发起窗**(设置窗是独立 BrowserWindow,退化成全窗广播 = 每扇窗都收一份别人的进度)。**两件宿主能力**走新立的 `wiring/plugins/host-ports.ts` 的 `configurePluginsHost`:`pickFile`(原生对话框,未注入即回 `Importing files into a plugin works on the desktop app only.`)与 `execCommand`(插件命令的子进程执行器 —— execa 是桌面这棵树的依赖,装配层动态 import 它会把它塞进 `dist/server/main.js` 那个单文件包);实现都在新的 `@onething/electron-host/plugins/ipc-host`(`main/ipc/plugins.ts` 因此仍然一行 `electron` / `ipcMain.handle` 都没有,旧的 `MAIN_PLUGINS_IPC_HOST_FORBIDDEN_PATTERNS` 原样守着)。**http 语义逐字不变**:六条读/开关面调 `server/plugin-catalog.ts` 的单槽端口 —— 端口里装的就是从前 `/api/plugins*` 六条 REST 背后的**同一批闭包**(`ServerPluginCatalogManager` 与它的 per-owner 表一行没搬,只是从 `OnethingRuntimeFacade` 的 `plugins` 一格改成注册进端口,shutdown 里**还原**而不是清空);`configGet` 的 http 分支 = 把迁移前 `platform/web.ts` 那段"从列表投影派生只读值"整块搬进域;其余写面按**管理器在不在场**(同 B 批 `collabRooms` 判例)—— 桌面内嵌 HTTP 面 `getPluginManager()` 已装配 → 与 ipc 同一条路,独立 `server:start` 没装配 → 回**逐字相同**的文案(含那条 501 的正文,语义没丢,只是从 HTTP 状态码变成结构化失败)。`core/runtime-facade.ts` 的 `RuntimePluginsAdapter` 随之删掉(只有一个实现者、一个读者)。**能力位 `pluginsManage`**(`platform/types.ts`,web 静态默认与 `normalizeServerCapabilities` 均 `false`、electron `true`):关着时 `plugins-client` 的十二条写面**根本不发请求**,就地返回与迁移前硬桩逐字相同的答案;读面(list/enable/disable/refresh/commands/executeCommand/configGet)不受它管 —— 它们在 web 上本来就打真路由。`platform/electron.ts` 那道**只查不修**的 payload 序列化自检跟着搬进 client,于是对两个宿主都跑。checker:`checkElectronHostOwnsPluginsIpcHost` 改写成反向棘轮 `checkPluginsDomainRidesTheRpcChannel`(同 providers/models/themes 判例;十九条常量、那只工厂、那个 exports 键都不许回来),四条 `checkRuntimeOwnsPlugins*` 原样保留。transport 收紧:channels **103→84**、bridge 864→775、http 806→730、web 1067→879、channels.ts 292→267,四壳 **2651**。验收:typecheck 0、boundary:gate ok(0 failed)、transport:gate ok、全量 **11245 绿**/8 skipped(1156 文件)、battery **264 runs / 0 mismatch**、build:check 绿、web:build 绿。真机(独立 `server:start`、隔离 store):`plugins.list` 回 log-monitor+note-skills、`plugins.commands` 空表、`configGet` 只读且 `readOnlyReason` 到位、`configGet(ghost)` 回 `Unknown plugin`、`install`/`pickFile`/`request`/`market`/`configSet` 逐条回那几句原话、`disable`→`list` 真落盘再 `enable` 回来、旧 `/api/plugins` 三条路由皆 **404**、`/api/capabilities` 无新字段(`pluginsManage` 是渲染侧静态位)。

> - **A1-a 已落地(立宿主壳路由 `shell:invoke` + 窗口系八域迁入;browser 20 条留 A1-b)**:基建对着 `rpc:invoke` 抄成对称的一套 —— 通道常量 `SHELL_INVOKE`(信封复用 `RpcRequest`/`RpcResponse`,不另造类型)、宿主侧派发表 `apps/electron/src/ipc/shell-registry.ts`(`registerShellDomain`/`dispatchShell`/`hasShellDomain`/`resetShellRegistryForTests`,形状照 `backend/rpc/registry.ts`:方法白名单、never-reject、`UNKNOWN_DOMAIN`/`UNKNOWN_METHOD`/`BAD_REQUEST` 三码、重复注册抛错、可逆反注册、**加载时零注册**;**本文件不 import electron**,所以八个域的处理者表在没有 Electron 的单测里就能钉)、主进程入口 `@main/ipc/shell-rpc.ts`(唯一一条 `ipcMain.handle`,`ShellDispatchContext = { callerId }` 从 `event.sender.id` **铸**、永不读信封 —— 与 `RpcDispatchContext` 同规矩)、preload 的 `shellInvoke` 与 `rpcInvoke` 并排、渲染侧 `platform/shell-client.ts`(`createShellClient(router) = createRouterClient(router, shellInvoke)`)+ 每域一个 `platform/<d>-client.ts`。**web 侧也是「契约 + 处理者表」而不是一坨 if**:`platform/shell-web/registry.ts` 同形派发,八个域各一份实现(`platform/shell-web/<d>.ts`),`web.ts` 只留一行 `registerWebShellDomains({ postJson, emitSearchAction, emitImagePreviewUpdate })` 与 `shellInvoke: dispatchWebShell`。
> **迁入八域 21 条通道**:`todo-plan-window` 7(open/hide/toggle/setPinned/minimize/zoom/**drag**)、`search-window` 4(toggle/close/setAnchor/**executeAction**)、`settings-window` 1、`window` 1(close)、`dialog` 1(showOpen)、`media-window` 3(saveAs/openPreview/openGallery)、`notify` 2(show/setBadge)、`deeplink` 2(ready/respond)。**每条的 invoke/推送判定**:`SEARCH_WINDOW_{SHOWN,GUIDES}`/`SEARCH_ACTION`/`SETTINGS_{NAVIGATE,CHANGED}`/`SYSTEM_THEME_CHANGED`/`IMAGE_{PREVIEW,GALLERY}_UPDATE`/`NOTIFY_ACTIVATE`/`DEEPLINK_REQUEST`/`TODO_PLAN_CHANGED`/`CONTEXT_SIZE_UPDATED`/`SESSION_MESSAGES_CHANGED`/`VOICE_RUNTIME_COMMAND` 全是**推送**,一条未动。三条**判定为不迁**:① `SEARCH_QUERY` 是 **invoke 但处理者是数据面**(桌面 `wiring/search/providers` 的 `executeSearch`,一行 electron 都不碰)——按判据该去 `rpc:invoke` 的 backend 域而不是壳路由,但 server 那侧是 per-owner 沙箱版(`createSearchProvidersForContext`),要像 files/tools 那样按 `context.transport` 分叉,**单独一批**,在此之前常量与手写 handler 原样留着;② `CLEAR_CHAT` 全仓**零引用**(无处理者、无 preload 包装、无调用点)——**删**;③ `UPDATE_SESSION_MAX_TOKENS` 桌面**从来没有处理者**、渲染层零调用点,只有 preload 包装 + web 一条 REST 镜像在空转——**删**(server 的 `POST /api/sessions/:id/max-tokens` 路由留着,mobile 面)。
> **两处判例值得记**:(a) `TODO_PLAN_DRAG_WINDOW` 从前刻意留在手写通道上避开"通用 RPC 的重封装",A1-a 复核后**照样搬** —— 壳路由每次调用只多一个信封字面量 + 一次 Map 查找,与 IPC 结构化克隆 + 一次 `setBounds` 不在一个量级,而"窗口系全部迁入"换来的是拖窗这条路也能从调用点直达处理者;(b) web 侧那些"做不到"的答复**逐字**沿用迁移前 `unsupported(method)` 生成的句子(连里面的旧方法名一起),而不是让域"没注册"——后者会让 `createRouterClient` 抛,而 `platformApi.toggleSearchWindow()` 这类未 await 的调用点从来只是拿一个 `{success:false}` 就算了,改成抛 = 当场 unhandled rejection。`callerId` 的四个真用例:`window.close`(关哪扇)、`search-window.toggle/executeAction`(从哪扇窗按的)、`notify.show`(点击唤回谁)。
> **顺带删掉的旧件**:`apps/electron/src/ipc/todo-plan.ts` 整只(exports 键一并删)、`registerElectronSearchIpcHandlers`(window-actions 换成 `getElectronSearchWindowFromCallerId`)、`registerElectronSettingsIpcHandlers`、`registerElectronMediaIpcHandlers`(两只 host 文件只留真实现:`showElectronOpenDialog` / `saveElectronMediaFileAs`)。checker 四条断言改指(search/settings/media/todo-plan 的 required 符号从"手写工厂 + 通道常量"改成"域注册 + callerId 解析"),`MAIN_*_IPC_HOST_FORBIDDEN_PATTERNS` 一条未放宽 —— `@main` 那三只适配文件仍然一行 `electron`/`ipcMain.handle` 都没有(注释里也不许写那串字面量,踩过一次)。**没有给 `shell-rpc.ts` 开任何例外**:checker 里本来就没有"@main 不许再长 handle 文件"的断言,它靠的是 transport 门的数值棘轮。
> **transport 收紧**:channels **84→62**(+1 `SHELL_INVOKE` −21 迁入 −2 死常量)、bridge 775→718、web 879→785、channels.ts 267→256,四壳 **3040→2489**(http 730 不变:窗口系从来没有 server 路由)。**新测试**:`ipc/__tests__/shell-registry.test.ts`(11 例:白名单/never-reject/三码/callerId 透传且信封里塞 callerId 无效/undefined→null/重复注册抛/陈旧 disposer 不误摘)、`ipc/__tests__/shell-domains.test.ts`(八域逐条断"方法集合 = router 上那几条"+ 打到注入操作 + 回形状)、`platform/__tests__/shell-web.test.ts`(11 例,文案逐字)、`web.test.ts` 五处改走 `shellInvoke`(顺带钉住 `shellInvoke` 真接上了那张表)、七只渲染层测试改 mock 域客户端。验收:typecheck 0、boundary:gate ok(0 failed)、transport:gate ok、全量 **11276 绿 / 1 抖动**(`backend/stores/sessions-delete-cascade` 满载下红、单跑绿,与本批无关,D2 记录里已列过同一只)、battery **264 runs / 0 mismatch / 0 appendFailures**、build:check 绿、web:build 绿。**遗留**:browser 20 条(A1-b);`SEARCH_QUERY` 迁 backend `search` 域(要 http 分叉);`apps/electron/src/ipc/shell-controller.ts` 那四条**字面量**通道(`shell:open-path`/`shell:open-external`/`app:get-data-path`/`window:set-button-visibility`)不在 `IPC_CHANNELS` 表上、本批未点名,是下一个明显的壳域候选。

> - **A1-b 已落地(browser 19 条 + shell-controller 四条字面量迁壳路由;`SEARCH_QUERY` 迁 backend `search` 域)**:A1-a 留下的三条遗留一次做完,基建一行没改 —— 加域 = 契约一份 + 宿主处理者一份 + web 处理者一份 + 客户端一份,四处,`web.ts` / `@main/ipc/handlers.ts` 的管道零改动,这正是 A1-a 要证的形状。
> **① browser 19 条**(`hydrate`/`createTab`/`closeTab`/`selectTab`/`navigate`/`goBack`/`goForward`/`reload`/`stop`/`setBounds`/`setVisible`/`pickElement`/`pickCancel`/`getSearchEngine`/`setSearchEngine`/`listProfiles`/`addProfile`/`removeProfile`/`switchProfile`)进 `browserRouter`。**判定是壳面不是数据面**:处理者动的是 `WebContentsView` —— 主进程里一扇真原生视图。`BROWSER_TABS_CHANGED` 是**推送**(一次合批的标签态广播),原地不动,`onBrowserTabsChanged` 仍在 platformApi 上。`setBounds` 是每帧一次的高频调用,与 A1-a 的 `todo-plan-window.drag` 同判例照迁。`callerId` **一个用例都没有**:浏览器服务是主进程单例,迁移前那批 handler 也从没读过 `event.sender`。旧可移植工厂 `apps/electron/src/ipc/browser.ts` 与它的 19 条 `ipcMain.handle` 整只删,`@main/ipc/browser.ts` 156→27 行(只剩注入服务 + 那条推送)。web 侧 19 条**注册了但老实回做不到**,句子与里面的旧方法名逐字沿用迁移前 `WEB_DESKTOP_ONLY_PLATFORM_METHODS` 生成的那句,能力位 `embeddedBrowser` 一格未动(仍 false)。
> **② shell-controller 四条字面量通道**(`shell:open-path` / `shell:open-external` / `app:get-data-path` / `window:set-button-visibility`)—— 它们**从来不在 `IPC_CHANNELS` 表上**,transport 门数都数不到。前三条进**新立的** `shellRouter`(`@shared/ipc/shell.ts` + `ipc/shell/shell.ts`),第四条进 A1-a 就有的 `windowRouter`:判据是"处理者动的是**发起窗本身**",与 `window.close` 同一条 `callerId` 规矩(`setCallerWindowButtonVisibility(callerId, visible)`,渲染层没有机会点名去改别人的窗)。`openPath` 的输出仍是 **Electron 那个错误串本身**(`'' = 成功`),不是信封 —— 这批只搬路不改 wire 形状;迁移前 `setWindowButtonVisibility` 那条 handler **没有回值**,搬到只有请求/响应面的 router 上补一条空回执(唯一调用点 `App.vue` 本来就 `.catch(() => {})`)。`ipc/shell-controller.ts` 与它的测试整只删;checker 的 `checkElectronHostOwnsShellOperations` 从"断那只工厂里有四条字面量"改成**反向棘轮** —— `LEGACY_SHELL_LITERAL_CHANNEL_PATTERNS` 扫 `apps/electron/src` 全树(走 `matchingCodeLines`,注释不算),那四条字面量一条都不许长回来。**与 `configureShellHost`(`runtime/src/shell/host-ports.ts`)是两件事**,没有合并:那个端口是"产品代码要打开一个目录"的注入口,这里是"渲染层要打开一个路径"的传输面,同一批 `openElectronPath` / `openElectronExternal` 被两条路各自调用,一直如此。
> **③ `SEARCH_QUERY` → backend `search` 域**:兑现 A1-a 的判定。它是继 files / tools / mcp 之后又一个**按 `context.transport` 分叉**的域 —— `ipc` 走桌面 `wiring/search/providers` 的 `executeSearch`(连 `executeOnethingSearchForIpc` 这层归一化都是同一个),`http` 走**新立的单槽端口** `backend/server/search-providers.ts`(判例逐字同 C2 的 `server/plugin-catalog.ts`:`createOnethingServerRuntime` 装配时把 facade 上那个**同一个闭包**注册进来,域原样调用,实现一行没搬)。端口没装(CLI daemon / 单测里的 http 上下文)= **结构化失败**,不偷偷降级去查桌面那份 —— 那会让一个网络调用者读到宿主机器上别人的会话。`RuntimeSearchAdapter.query` 与 `POST /api/search/query` 一起删;**`executeAction` 与 `POST /api/search/actions` 留着**,它是**窗口活**在 server 侧的对应物(A1-a 的 web 壳 `searchWindowRouter.executeAction` 打的就是它),`SEARCH_ACTION` 推送同理不动。`apps/electron/src/search/ipc.ts` 从此只剩窗口活,checker 那条断言换成反向棘轮(这只文件里不许再有 `SEARCH_QUERY` / `ipcMain.handle`)。
> **transport 收紧**:channels **62→42**(−19 browser −1 `SEARCH_QUERY`;那四条字面量本来就不在表上,所以门数不到它们的消失)、bridge 718→674、http 730→726、web 785→758、channels.ts 256→234,四壳 **2489→2392**。**新/改测试**:`rpc/__tests__/search-domain.test.ts`(4 例:ipc 分支逐字打到 `executeSearch`+非法 category 归一、http 分支只打端口且带 owner 上下文、缺端口结构化失败;读 `data` 前先看 `ok`)、`ipc/__tests__/shell-domains.test.ts` 加 browser(方法集合 = 19 条、逐条信封→位置参数、抛错折成旧兜底、**懒单例不被提前求值**)与 shell 两域 + window 域补 `setButtonVisibility`、`platform/__tests__/shell-web.test.ts` 加 browser 19 条与 shell 三条(文案逐字)、`http.test.ts` 两条搜索用例改走 `POST /api/rpc`(断言一条没减,含 alice/bob 归属隔离)、`web.test.ts` 两处改走 `shellInvoke`、`app-rpc-features` 域数 +1(`rpc:search`,排在 plugins 之后、自进化之前)、四只渲染层测试改 mock 域客户端(BrowserStartPage / usePickerOrchestration / ChannelsSettingsTab / CollabBoardPanel / TodoPlanPanel)。**遗留**:窗口系已全部迁完,壳面只剩推送;`RuntimeSearchAdapter` 只剩 `executeAction` 一格,它随将来"执行动作也归窗口域的 server 对应物"一并收口时才会消失。

> **08-24 I4-缝收口(S 线,用户 /goal:设计方案并修复)**:病历=缝(行为端口)开得比变化轴多、且鸭子口无声明边(tsserver 的 Go to Implementation 只认 implements/extends/显式类型标注;`EventBus` 满足 `CoreSessionCommandEmitterLike` 却不声明 implements,gi 返回空)。目标态:**每道缝要么有租金要么删;留下的缝必须有 IDE 能走的边;全仓追踪只剩两种断口**(生成式 client → router 常量 Shift+F12;总线 → 词汇常量 Shift+F12)。四期,顺序 S1‖S3 → S2 → S4,每期五门 + battery:
> - **S1 缝审计**(零改动):盘点全部带方法成员的端口接口(core/runtime 的 `*Like`/`*Port`、引擎五端口、configure*Host),每口一行:实现数/调用者数/变化轴/判定(留+补边 | 删内联 | 另册),产出 `docs/audit/port-seam-audit-2026-08.md`。
> - **S3 删假入口**:40 个 `registerXxxRpcDomain` 包装生产零调用(真挂载在 `rpc/index.ts` 域表),删包装、37 只域测试改直调 `registerRouterHandlers(router, handlers)` —— 与生产同一底层调用,Shift+F12 引用列表去噪。
> - **S2 装门**:S1 判"留"的缝,类实现补 `implements`、对象字面量实现补显式类型标注(`satisfies` 不产生 tsserver 边,不用);`scripts/probe-go-to-implementation.mjs` 用 `getImplementationAtPosition`(必须 `realpath: ts.sys.realpath`)对清单逐条验证,全绿为验收。
> - **S4 微缝内联**:S1 判"删"的(单实现、单调用者、无层向理由),改直接类型引用,每内联一个链路少一跳;最后做,门兜底。
> **S1 已落地(08-24,零源码改动)**:`docs/audit/port-seam-audit-2026-08.md`(756 行)。方法=真 `ts.Program`(tsconfig.node,2920 源文件)+ TypeChecker 三遍 AST(implements/satisfies 边、对象字面量 contextual type、标识符供体 contextual type)。初筛 682,入表 **516**(纯数据形状 166 跳过):**留+补边 254 / 留已通 141(27%)/ 删内联 82 / 另册 71**(死口 1、单实现多消费 12、零生产实现 58)。病历确认:`CoreSessionCommandEmitterLike` 唯一生产实现 `backend/events/event-bus.ts:8`,零 implements。引擎五端口全"留+补边"(生产填充点是 stream-engine-bound.ts 五个就地字面量);77 个 configure* 零死口。**口径已拍(Fable):参数包(Options/Config/Params,或全部引用在 param 位)被 N 个调用点各自填一次不算变化轴** —— "实现"是提供行为的一方,调用点填参不是实现;该口径影响 133 行,写在审计 §1.1。另册线索:三个 SQLite 适配口(store-helpers/session-repository/session-message-runtime,24 个方法签名)是 sqlite 退役化石,并入 S4 删除清单。优先内联前 10(§7):CoreIPCEmitter(19 法,SEND_MESSAGE 出口)、CoreEventBusLike、CoreStreamProcessor、OnethingAgentLoopRuntimeAdapters(22 法零外部消费)、CoreToolRuntimeContext 等。
> **S3 已落地(08-24,与 S1 同 commit `47e7933e`)**:41 个 `registerXxxRpcDomain` 包装全删(生产零调用,41 域文件 +77 −247),37+2 只测试改直调 `registerRouterHandlers(xRouter, xRpcHandlers)`(与 vi.resetModules 兼容:从取 dispatchRpc 的同一 import 取);checker 7 域断言搬到真挂载点并收紧为 `includes('<x>Router, <x>RpcHandlers')`;transport 数值一字未变。
> **S2 已落地(08-24)**:256 行工单 **253 道缝接通 gi**(核对法=真跑 `getImplementationAtPosition`:施工前 14 通 → 施工后 253 通),116 文件,四类手法:就地实参/属性提成带标注 const(约 200 处)、鸭子类补 `implements`/`extends`(EventBus/Scheduler/StreamProcessor/PluginAPI/三个 PermissionBridge…)、factory 返回类型显式化、type alias 改 interface(**实测 gi 不跟随类型别名**——IPCEmitter 等三口改 `interface extends` 边才出现)。35 道鸭子 logger 口靠 `console-port.ts` 补 35 条 extends 接通(过渡表,Core*Logger 统一成 Logger 时随文件删)。未接成 3:CoreFileWatcher(供体是 node FSWatcher 平台类型)、CoreSessionCacheAdapter(LRUCache 的 K 无 string 约束,接上须改接口=S2 禁区)、StreamEngineStreamsAdapter(成员是逐字重复的就地结构类型,留 S4 改引用具名口)。改判 2:gateway 的 LoggerRootOptions 实现点补边会破 gateway 只许 `@onething/core/gateway-runtime` 一条说明符的边界规则,回退(该口已由 runtime/backend 两实现点接通);审计 gi 列是启发式、若干行实测已通。探针 `scripts/probe-go-to-implementation.mjs` 58/58 PASS(病历条注明语义:gi 停在方法名上找的是"方法实现",继承的 emit 不在其列;边装在口一级,探针停在口引用与注入点)。门:typecheck 0 / boundary 0 / transport 不变 / log·session gate ok / battery 238/0;全量测试本机高负载抖动(失败集每轮不同、单跑全绿),stash 基线同机 103 failed 远差于本分支 1–9,证与纯类型改动无关。
> **S4 已落地(08-24,S 线收口)**:内联 **41 缝 + 3 化石 + 1 条 S2 遗留**(51 文件,净删 162 行):§7 前十完成 6(CoreEventBusLike/CoreToolRuntimeContext/CoreToolDirectExecutionCallbacks/CoreAgentLoopDirectToolExecutionContext/ApplyAgentLoopFinishChunkWithAdaptersOptions/BoundJobRegistry→ReturnType),core 13 / runtime 28;`CoreSessionSqliteMutationAdapters` 零引用删净,另两个 SQLite 化石具名口删、形状原地内联(`sqlite?:` 字段与 29+9 处分支是活消费路径——两只测试真在填——拆它=独立的 SQLite 退役工程,不归 S4);`OnethingProductStreamRuntimeOptions.streams` 改引用 `StreamEngineStreamsAdapter`。**未做 35,逐条有理由**:28 条多类型位(内联即复制,含 §7 #3 CoreStreamProcessor 跨包双泛型、#4 OnethingAgentLoopRuntimeAdapters 同文件 5 位)、3 条会破护栏(#10 McpToolBridge 内联须 import `.wiring` 破 I3;OnethingProviderRegistry/OnethingSearchRuntimeAdapters 是 checker 必需符号)、4 条 provider 方言口(用户 Wire×Dialect 在途,gi 本就通)。冲突 2(PluginConfigAccess/IPCEmitter 是 S2 刚装的门,回退保 S2);改判 3(ConsoleLikePort 实测 36 消费者、AgentExecutor 三工厂真多实现、CoreStorageProvider 有 implements)。审计 §2 写 82 与表行 81 差 1 是审计自身计数出入。门:typecheck 0 / boundary 0 / transport 42/2392 不变 / battery 238/0 / build:check 绿 / 探针 58/58;全量 10 失败全为本机冷启动超时(+30s 门 79/79 绿;stash 基线同机 31 failed 更差)。
> **S 线终局**:S1 审计(516 缝、gi 通过率 27%)→ S3 删 41 假入口 → S2 装门 253 缝(探针 58/58)→ S4 内联 45 处;**gi 可达率从 27% 到"留"缝全通**,追踪断口收敛为两种(生成式 client → router 常量;总线 → 词汇常量),各配标准把手。遗留:SQLite 退役工程(sqlite 字段+分支+core 四口)、console-port 过渡表随 Core*Logger 统一删、LRUCache K 约束、CoreFileWatcher 平台类型,均另立不归 S 线。
...
3. 批次顺序:
   ...
   - P4c 其余 ~24 工厂域按"低风险→高风险"排(建议:themes/scratchpad/app-state 等小域先,
     ...
     **P4 至此:所有带工厂/裸写的数据面域已迁完**,剩窗口系残留集(#10:browser/shell/window/search/notify/deeplink/todo-plan 窗口/
     media 3 条/settings 2 条;chat resumeAfterToolConfirm 已随 #21 于 08-22 删除)、plugins 写面(#16)、mobile REST(#32);
     **terminal(#18)已于 08-22 D2 迁完**(七条请求面走 `terminalRouter`,两条推送留注入广播器,能力位仍默认关)。
...
| 10 | **(新)窗口系通道的典型形态**(todo-plan minimize/zoom/drag 等操作 BrowserWindow 的通道):宿主壳自己的 `shell`/`window` 路由 + sender 上下文 vs 维持手写 | P4 终态前统一成一个宿主壳路由;transport 门区分"域通道"(禁增)与"窗口系"(白名单) | 已做(08-23 A1-a + A1-b:窗口系**全部**迁入 `shell:invoke` —— A1-a 八域 21 条 + A1-b browser 19 条与 shell-controller 四条**表外字面量**(前三条进新立的 `shellRouter`、`window:set-button-visibility` 进 `windowRouter`);`SEARCH_QUERY` 判为数据面已迁 backend `search` 域(带 http 分叉)。壳面**只剩推送** N 条(`SEARCH_WINDOW_{SHOWN,GUIDES}`/`SEARCH_ACTION`/`SETTINGS_{NAVIGATE,CHANGED}`/`SYSTEM_THEME_CHANGED`/`IMAGE_{PREVIEW,GALLERY}_UPDATE`/`NOTIFY_ACTIVATE`/`DEEPLINK_REQUEST`/`TODO_PLAN_CHANGED`/`BROWSER_TABS_CHANGED`/…)加 `RPC_INVOKE`/`SHELL_INVOKE` 两条通用通道;门仍是**一把数值棘轮**,没有为窗口系开白名单) |
...
| 16 | **(新,P4c)plugins 迁 router = web 长出 install/uninstall/config 写/market 全套真能力**,与"插件只在桌面执行(方案 A)"冲突;server 现有 enable/disable 写路由改的是 `owners/<uid>/<wid>/plugin-store` 另一棵树 | 不迁写面;只迁只读面(list/commands)并保留桌面专属写面,或整域留到方案 A 重议 | 已做(08-23 C2:**整域迁 + web 写面能力位 `pluginsManage` 默认关**。读/开关面在 http 上仍走 server 那棵只读镜像树 —— 闭包一行没搬,只是从 facade 的一格改成 `server/plugin-catalog.ts` 的单槽端口;写面按**插件管理器在不在场**判定,独立 server 回迁移前 `platform/web.ts` 那批**逐字相同**的文案。放开能力位是一次独立拍板) |
...
| 18 | **(新,P4c)terminal 内核已在 app 层,技术上可迁,但 `terminal:false` 政策冻结** | 维持冻结,不迁 | 已做(08-22 D2:七条请求面迁 `terminalRouter`;**能力位仍默认关**,且闸从渲染侧上移到**域的 http 分叉**——七条一律结构化拒绝、不求值懒单例;放开 = 去分叉 + 给 server 接推送广播器) |
...
| 21 | **(新,P4c)chat 的 `resumeAfterToolConfirm` invoke 往引擎递 sender;web 同名方法早已走命令总线** | 不迁 router,删 invoke、桌面也走命令总线 | 已落地(2026-08-22)—— 渲染层零调用者,「桌面也走命令总线」无物可走,执行口径收成**删整条 invoke 链 + 删 platform 方法**:通道常量、两只手写工厂(`apps/electron/src/ipc/chat.ts` / `@main/ipc/chat.ts`)、preload 包装、`platformApi` 声明与 web 实现、runtime `sessions/tool-confirmation.ts` 全删;core 的 `command:resume-after-confirm` + `handleResumeAfterConfirm` 保留(引擎能力,退役另列待拍) |
...
| 37 | **(新,C2/D2 并排查出的判例分叉)插件写面在 http 上按"插件管理器在不在场"分叉(桌面内嵌 HTTP 面上持 Bearer 的浏览器可经 `/api/rpc` 真装插件,只靠渲染侧 `pluginsManage` 位遮 UI);terminal 则在域里按 transport 一律拒(理由:拿到 Bearer 的浏览器渲染侧位挡不住)。两条都是执行中判定,语义并不一致** | A(推荐,与 B 批 collabRooms 同判例)统一为"域里按服务在不在场"——terminal 去掉 http 一律拒、改为 PTY 面在场即放行(server 本就不装配);B 统一为"http 一律拒直到单独放开"——plugins 写面在 http 上也拒;C 维持现状各守各的 | 待拍 |
