/**
 * 检索语料的**脱敏规则**(检索重建 S0,`docs/design/search-index-2026-09.md` §10)。
 *
 * 语料是从**真库**(`~/.onething/sessions/*`)抽出来的真实对话,要进仓当夹具
 * (`packages/core/search/__tests__/fixtures/corpus.json`)。所以脱敏不是「顺手
 * 做一下」,它是这份夹具能不能进仓的前提。
 *
 * 这个文件里只有**纯函数** —— 没有 IO、没有 store、没有任何仓内依赖:
 *  · 抽取脚本(`scripts/search-corpus-extract.mjs`)拿它把正文洗一遍;
 *  · 单测(`scripts/__tests__/search-corpus-redact.test.mjs`)逐条守每一条规则;
 *  · 夹具自检(`packages/core/search/__tests__/fixtures.test.ts`)拿**同一份**函数
 *    在成品语料上再跑一遍 —— 结果必须**逐字相同**(幂等),那就等于「一条漏网都
 *    没有」。这条恒等式是这份文件唯一的验收判据,所有占位符因此都不许再被自己
 *    的规则命中。
 *
 * ## 规则表(顺序有意义,见每条的说明)
 *
 ...
 *
 * 放过的段(见 `isPublicIpv4`):私网与链路本地、回环、`0/8`、组播与其上的保留段
 * (首段 ≥ 224,顺带盖住 `255.x` 掩码)、四条公共 DNS,以及文档 / 基准测试专用段
 * (`1.2.3.4`、`198.18/15`、三条 TEST-NET)—— 这些是**公开常量**,不是谁的地址,
 * 洗掉只会让语料变哑。任一段 > 255 的也不算 IP(版本号、日期都长这样)。
 */
...
/** 规则 id 表(单测与报告按它逐条走)。 */
export const REDACT_RULE_IDS = RULES.map(rule => rule.id)
...
export function redactText(text) {
  ...
  for (const rule of RULES) {
    ...
    out = rule.accept
      ? out.replace(pattern, match => (rule.accept(match) ? rule.replace : match))
 ...
 */
export function findRedactionHits(text) {
