 ...
 * 那一份会话 / 文件 / 提示词表(`wiring/search/providers` 的 `executeSearch`),
 ...
 * `rpc/domains/search.ts` 在 `transport === 'http'` 那一支上原样调用。
 * **实现一行没搬、语义一字未改** —— 换的只是入口(判例逐字同 C2 的
 * `server/plugin-catalog.ts`)。
 *
 * 未注入 = 这台进程没有 server 运行时(桌面 IPC、CLI、单元测试)。那一支上域走的是
 * 桌面那条真路(`executeSearch`),永远不会问到这里。
 *
