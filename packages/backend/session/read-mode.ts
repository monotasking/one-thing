 ...
 *
 * ## `ONETHING_SESSION_HYDRATE` —— 已退役(F 线 F4-a,§16.12)
 *
 * S3w-1 给**冷加载补水**开的 `messages | projection` 岔口,批 3 把默认翻到
 * `projection`,`messages` 留作回滚杆。**F4-a 把它烧掉了**(§16.11 拍板 5,
 * 用户 2026-08-27 裁定)。理由与 `ONETHING_SESSION_READ` 那根杆逐字相同:批 6a
 * 起抄本停写、批 6b 起写代码已删,所以 ①停写之后出生的会话没有抄本,扳过去补
 * 出来的是空;②停写之前出生、之后又聊过的会话,抄本冻在停写那一刻,扳过去等于
 * 把那之后的历史补丢。批 6b 当时已经在这里写下"**不要扳它**" —— 一根注着
 * "不要扳"的杆不该留在代码里假装还能回滚。回滚 = `git revert`。
 *
 * 补水从此**无条件**走投影(`hydrate.ts` 只剩"这条会话的事件里折不折得出历史"
 * 一个判据)。那道翻默认时立的合同门仍在:`bun run sessions:hydration-contract`
 * 对全量真机会话断言「投影补水 + rehydrate + sanitize ≡ 抄本 + rehydrate +
 * sanitize」(翻档那天 436 会话 pass 417 / fail 0 / 0 新类)。
 *
 * (`ONETHING_SESSION_TRANSCRIPT` 也已退役,见文件末尾的墓志铭。)
 *
 * 于是本文件不再有任何档位 —— 只剩下面那一句跨进程的告警。
 */
