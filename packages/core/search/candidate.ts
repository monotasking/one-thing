...
export interface SearchPage {
  ...
  /**
   * **这一页只扫到一半**(09-07 事故的第三条修:`docs/design/search-index-2026-09.md` §7.1)。
   *
   * 预算到点时一路枚举外部资源的能力手上往往已经有东西了。从前只有两种结局
   * ——「答完」或「超时 = 整组作废」—— 于是慢盘上那一路每次都是一句「没搜成」,
   * 而它明明扫到了三十个文件。这一格是第三种结局:**交已经扫到的那些,并说清楚
   * 没扫完**。缺席 = 这一页是完整的(不是「不知道」:一个不报 partial 的能力就是
   * 在说自己答完了)。
   *
   * `fanout` 认这一格:标了 partial 的页即使空着也**不算超时错误** —— 它答过了。
   */
  partial?: boolean
}
...
export interface GroupResult {
  ...
  error?: string
  /**
   * **这一组没跑,因为不挑那一档不等它**(09-07 事故的第二条修)。
   *
   * 起因是真机上一次「不挑」的搜索永远不回:一路去枚举外部资源的能力扫了 18GB
   * 还没扫完,而整发要收齐所有组才答。判据**在自述上**(`manifest.kind`),不在
   * 任何一个能力的名字上:一路自称去外部枚举,它就不该决定别人什么时候上屏。
   *
   * 它与 `error` 是两件事:`error` 说「问过了,没答上来」,这一格说
   * **「这一次根本没问它」** —— 所以调用方要么单独去问一次(壳就是这么做的),
   * 要么如实说这一档这次没参与。`page` 恒缺席。
   */
  deferred?: boolean
}
