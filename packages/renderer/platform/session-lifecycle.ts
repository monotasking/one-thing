/**
 * **会话生命周期的全局订阅口**(共享层读侧补齐 E 批)。
 *
 * ## 为什么它不是 `platformApi` 上的一格
 *
 * 因为它不需要传输面出一分力。`session:event` 那条推送面本来就把**所有会话**的
 * 事件送到渲染层(桌面 IPCBridge 用 `onAnySessionAny`,web 用 `?sessionId=*` 的
 * SSE),缺的只是"从那条流里认出建/删这两件事"的那一层折叠 —— 折叠是渲染层的活,
 * 不是通道的活。所以这里是**壳外一个模块**,`platformApi` / preload / 四壳
 * 一行不动(判例同 `logs-client.ts` / `session-command-client.ts`)。
 *
 * ## 两件事各自的产地
 *
 * | 事件 | 从哪儿来 |
 ...
 * ## 归属过滤在服务端,不在这里
 *
 * web 侧那条通配订阅逐条问 `canReadSession`;这里看到的每一条都已经过闸。
 * 渲染层**不许**自己再判一次 —— 判据只有一把尺。
 */
 ...
 *
 * 纯函数、不碰 `platformApi` —— 单测因此不需要造一个宿主。
 */
 ...
 *
 * `platformApi` 是按访问解析的代理(electron 桥 / web fetch 二选一),所以这里
 * 在**调用时**取 `onSessionEvent`,不提前快照。
 */
export function onSessionLifecycle(
  callback: (event: SessionLifecycleEvent) => void,
): () => void {
  return platformApi.onSessionEvent(envelope => {
