 ...
 * (`transformers-wasm`)只在这里挂一个**工厂** —— `@huggingface/transformers` 本体
 * 要到 `create()` 之后的 `ready()` 里才动态 import,所以「注册」这件事零成本、
 * 也不把 wasm 拖进任何 bundle 的启动路径。
 */
...
export {
  ...
  E5_SMALL_MAX_TOKENS,
  createTransformersWasmEmbedder,
  transformersWasmEmbedderFactory,
} from './transformers-wasm.js'

import { fakeEmbedderFactory } from './fake.js'
import { listEmbedders, registerEmbedder } from './registry.js'
import { transformersWasmEmbedderFactory } from './transformers-wasm.js'

/** 幂等。加第三条嵌入器 = 这里多一行,别处一个字不改。 */
export function registerBuiltinEmbedders(): void {
  const known = new Set(listEmbedders())
  for (const factory of [transformersWasmEmbedderFactory, fakeEmbedderFactory]) {
