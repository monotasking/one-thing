 ...
 * `session_message` 在策略表里是 `ask`(合表,2026-09-10:往别人的会话投一条消息
 * 是真有后果的 —— 收件人是另一条会话里的那个人 / 那个 agent,投出去就收不回来)。
 * 那不是合表带来的行为变化:判定核这一侧一直在问它,合表只是让策略表那一行
 * (原本写着一句不作数的 `silent`)说了真话。效果同时进审计。
 *
 * ## 场子门在哪
 *
