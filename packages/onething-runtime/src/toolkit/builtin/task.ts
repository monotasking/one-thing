 ...
 *
 * ## `session_spawn`:报效果,但**不弹卡**
 *
 * 旧实现是 `permissionGuard: 'safe'` + `autoExecute: true`,派工不弹权限卡。新实现
 * 按 §4 家族表报一条 `session_spawn`,而那条 kind 在策略表里是 **`silent`**
 * (R3a 复盘裁定,见 `core/toolkit/effects.ts`):派工的本义就是"派出去继续干",
 * 风险由两道既有的闸兜住 —— 每条会话的并发上限,以及子会话自己的权限卡(它继承
 * 调用方的权限模式,每一次真实副作用照常审批)。在这里再要一次审批,只会让用户
 * 为「要不要开始」点一次同意,再为那条会话里的每一件事点第二次。
 *
 * 那为什么还要报它:**审计**。旧路里"这一回合派出去过一条会话"只留在日志里,
 * 谁都不能从工具调用本身读出来。效果保留(且仍是屏障 —— 两次并发登记会把并发闸
 * 算错)让它成为一条可查的证词。用户可见行为因此与旧路逐字相同。
 */
