/**
 * `ask_user` 的行为金标(R3a 起的对拍 suite,R4b 转成金标 —— 见 time.test.ts 的
 * 头注释:旧实现删除前对拍是绿的,所以快照里记的就是那份旧行为)。
 *
...
describe('golden: ask_user', () => {
   ...
   * (metadata.outcome = 'aborted' + `ASK_USER_ABORTED_REASON`);新路里它是内核的
   * `Outcome.aborted`。撤回登记照旧发生(下面那条钉的就是它)。
   */
  it('取消:信号先响 —— 结成 aborted', async () => {
