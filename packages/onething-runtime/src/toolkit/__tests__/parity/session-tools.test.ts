/**
 * R3a 对拍 —— `goal` / `task`。
 * 旧 `tools/builtin/{goal,task}.ts` vs 新 `toolkit/builtin/{goal,task}.ts`。
 *
...
import { createGoalTool as createLegacyGoalTool } from '../../../tools/builtin/goal.js'
import { createTaskTool as createLegacyTaskTool } from '../../../tools/builtin/task.js'
...
describe('parity: goal', () => {
  it('spec 与旧工具逐字对齐', () => {
    const legacy = createLegacyGoalTool(goalAdapters(activeGoal()))
  ...
  for (const fixture of FIXTURES) {
    it(`模型文本与渲染信息一致:${fixture.name}`, async () => {
      const legacyTool = createLegacyGoalTool(goalAdapters(fixture.goal))
  ...
  it('边界:第二次 complete 真的完成(自检通过之后)', async () => {
    ...
    const legacyTool = createLegacyGoalTool(goalAdapters(activeGoal()))
    const { ctx } = legacyContext()
  ...
  it('错误:complete 缺 reason —— 两边同一句话', async () => {
    const args = { action: 'complete' }
    const { ctx } = legacyContext()
    const legacyError = await createLegacyGoalTool(goalAdapters(activeGoal()))
      .execute(args as never, ctx as never)
      .then(() => undefined, (error: unknown) => error as Error)

    const run = await runNewTool(createGoalTool(goalAdapters(activeGoal())), args)
...
describe('parity: task', () => {
  ...
  it('spec 与旧工具逐字对齐(描述 / schema / 并发档)', () => {
    const legacy = createLegacyTaskTool(ports(OK))
  ...
  for (const fixture of FIXTURES) {
    it(`模型文本与渲染信息一致:${fixture.name}`, async () => {
      const legacyTool = createLegacyTaskTool(ports(fixture.outcome))
      const { ctx } = legacyContext()
      const legacy = await legacyTool.execute(fixture.args as never, ctx as never)
