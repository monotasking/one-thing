/**
 * R3a 对拍 —— `practice` / `radio`。
 * 旧 `tools/builtin/{practice,radio}.ts` vs 新 `toolkit/builtin/{practice,radio}.ts`。
 */

import { describe, expect, it } from 'vitest'
import { createPracticeTool as createLegacyPracticeTool } from '../../../tools/builtin/practice.js'
import { createRadioTool as createLegacyRadioTool } from '../../../tools/builtin/radio.js'
...
describe('parity: practice', () => {
  it('spec 与旧工具逐字对齐', () => {
    const legacy = createLegacyPracticeTool(practiceAdapters())
  ...
  for (const fixture of FIXTURES) {
    it(`模型文本与渲染信息一致:${fixture.name}`, async () => {
      ...
      const legacy = await createLegacyPracticeTool(adapters).execute(fixture.args as never, ctx as never)
  ...
  it('错误:适配层抛错原样冒泡', async () => {
    ...
    const { ctx } = legacyContext()
    const legacyError = await createLegacyPracticeTool(boom).execute({ action: 'query' } as never, ctx as never)
...
describe('parity: radio', () => {
  ...
  it('spec 与旧工具逐字对齐', () => {
    const legacy = createLegacyRadioTool(radioAdapters())
  ...
  for (const fixture of FIXTURES) {
    it(`模型文本与渲染信息一致:${fixture.name}`, async () => {
      ...
      const { ctx } = legacyContext()
      const legacy = await createLegacyRadioTool(adapters).execute(fixture.args as never, ctx as never)
