/**
 * R3a 对拍 —— `web_search` / `web_open`。
 * 旧 `tools/builtin/web-search/{index,open}.ts` vs 新 `toolkit/builtin/web-{search,open}.ts`。
 *
...
import { createWebSearchTool as createLegacyWebSearchTool } from '../../../tools/builtin/web-search/index.js'
import { createWebOpenTool as createLegacyWebOpenTool } from '../../../tools/builtin/web-search/open.js'
...
import { annotationsOf, legacyContext, modelTextOf, normalizeDetails, partialsOf, runNewTool } from '../support.js'
...
describe('parity: web_search', () => {
  const adapters = () => ({ providers: { fake: fakeProvider() }, getFetch: fakeFetch })

  it('spec 与旧工具逐字对齐(描述 + JSON schema + 并发档 + 渲染档)', () => {
    const legacy = createLegacyWebSearchTool(adapters())
  ...
  for (const fixture of FIXTURES) {
    it(`模型文本与渲染信息一致:${fixture.name}`, async () => {
      const legacyTool = createLegacyWebSearchTool(adapters())
      const { ctx, record } = legacyContext()
      const legacy = await legacyTool.execute(fixture.args as never, ctx as never)
      ...
      // 进度事件仍然存在(旧的 updateResult 条数与新的 partial 条数相同)
      expect(partialsOf(run).length).toBe(record.updates.length)
    })
  }

  it('错误:没有配置任何 provider —— 两边同一句话', async () => {
    const { ctx } = legacyContext()
    const legacyError = await createLegacyWebSearchTool({}).execute({ query: 'x' } as never, ctx as never)
...
describe('parity: web_open', () => {
  const adapters = () => ({ getFetch: fakeFetch })

  it('spec 与旧工具逐字对齐', () => {
    const legacy = createLegacyWebOpenTool(adapters())
  ...
  for (const fixture of FIXTURES) {
    it(`模型文本与渲染信息一致:${fixture.name}`, async () => {
      const legacyTool = createLegacyWebOpenTool({ getFetch: fixture.fetch })
      const { ctx, record } = legacyContext()
      const legacy = await legacyTool.execute(fixture.args as never, ctx as never)
      ...
      expect(run.outcome.kind).toBe('ok')
      expect(modelTextOf(run.outcome)).toBe(legacy.output)
      expect(annotationsOf(run).map(entry => entry.title).at(-1)).toBe(legacy.title)
      expect(normalizeDetails(annotationsOf(run).at(-1)?.details)).toEqual(normalizeDetails(legacy.metadata))
      expect(partialsOf(run).length).toBe(record.updates.length)
