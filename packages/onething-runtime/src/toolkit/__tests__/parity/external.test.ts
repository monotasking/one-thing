 ...
 *
 * 这两只没有"旧工具对象"可比:插件工具是 `app/plugins/api.ts` 用 `Tool.define`
 * **现包**出来的,MCP 工具在旧路里根本不是一个工具(它是
 * `core/engine/direct-tool-execution.ts` 里的一支 if)。所以对拍的对象是**那两段
 * 旧代码本身**:
 *
 *  - 插件:同一份 def 分别喂给 `Tool.define(...)+executeCorePluginTool`(旧包法,
 *    在这里原样复刻)与 `new PluginTool(...)`;
 *  - MCP:新树复制过来的 `buildMcpPermissionPlan` 与 core 里那一份
 *    (`@onething/core/engine` 的 `buildMCPPermissionPlan`)**逐组比对**——
 *    复制一份的代价就是这条测试,它保证两份不会分家。
 */
...
import { Tool as LegacyTool } from '../../../tools/tool.js'
...
/** 旧包法:`app/plugins/api.ts` 那一段,原样复刻(恒 permission-gated)。 */
function legacyPluginTool(definition: ReturnType<typeof pluginDefinition>) {
  return LegacyTool.define<typeof PluginParams, EchoMetadata>('plugin:demo:echo', {
    ...
    async execute(args: unknown, rawCtx: unknown) {
      const ctx = rawCtx as Record<string, unknown>
      return executeCorePluginTool(definition as never, args as never, {
        ...
        metadata(input: { title?: string; metadata?: Partial<EchoMetadata> }) {
          ;(ctx.metadata as (input: unknown) => void)?.(input)
        },
      } as never) as never
    },
  })
...
describe('parity: PluginTool', () => {
  it('spec 与旧包法逐字对齐(描述 / schema / 并发档),效果是一条 plugin_exec', () => {
    const definition = pluginDefinition('ok')
    const legacy = legacyPluginTool(definition)
    const tool = newPluginTool(definition)
    expect(tool.spec.description).toBe(legacy.description)
    expect(tool.spec.title).toBe(legacy.name)
  ...
  for (const fixture of FIXTURES) {
    it(`模型文本与渲染信息一致:${fixture.name}`, async () => {
      ...
      const legacy = await legacyPluginTool(definition).execute(fixture.args as never, ctx as never)
      ...
      // 旧 ctx.metadata() 的那一条与结果标题那一条都在
      const titles = annotationsOf(run).map(entry => entry.title)
      expect(titles[0]).toBe(record.metadataCalls[0]?.title)
      expect(titles.at(-1)).toBe(legacy.title)
  ...
  it('错误:插件抛错 —— 两边同一句话', async () => {
    const definition = pluginDefinition('throw')
    const { ctx } = legacyContext()
    const legacyError = await legacyPluginTool(definition).execute({ value: 'x' } as never, ctx as never)
      .then(() => undefined, (error: unknown) => error as Error)

    const run = await runNewTool(newPluginTool(definition), { value: 'x' })
...
describe('parity: McpTool —— 复制过来的权限计划与 core 那一份逐组相等', () => {
