 ...
 *
 * 产品层(`packages/onething-runtime/src/**`,不含 `src/app`)不能 import
 * `@onething/app` —— 依赖方向是单向的(product ← assembly ← hosts)。所以这里
...
/**
 * 装配层的接线口。`@onething/app/logging` 的 `configureLogging()` 调它一次。
 ...
 * 装配层的 `@onething/app/logging` 原样再导出,老调用点一行不改。
 */
