 ...
 *  - `imageDetail`(值域按家:openai/openrouter 是 `auto|low|high`,
 *    deepseek 多一个 `original`)—— 由 codec 写进**每一个** `image_url.detail`。
 *
 * **`searchParameters` 不在这条线上**:它是 xAI Live Search 的开关,而 xAI 的
 * 两条通路(grok / grok-oauth)在 P4-4 整家搬到了 openai-responses。那张嵌套
 * 白名单跟着搬进 `xai-search-parameters.ts`,由
 * `openai-responses-provider-options.ts` 使用 —— 这条线上没有一家认它,所以
 * 收到 `searchParameters` 一律当认不出的键丢弃并留痕。
 *
 * ## 为什么留痕只在一处
 *
