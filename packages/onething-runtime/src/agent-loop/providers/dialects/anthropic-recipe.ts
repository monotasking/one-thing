 ...
 * 静态 `capabilities` 因此仍是纯传输声明(与今天 `options.capabilities ??
 * CLAUDE_CAPABILITIES` 一致),而 per-model 的账本覆盖仍由 `factory.ts` 的
 * `withPerModelCapabilities` 在外面盖一层 —— 两次投影是幂等的
 * (`base/__tests__/architecture.test.ts` 逐用例守着两份等价)。
 */
